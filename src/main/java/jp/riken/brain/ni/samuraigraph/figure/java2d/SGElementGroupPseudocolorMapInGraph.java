...
public class SGElementGroupPseudocolorMapInGraph extends SGElementGroupPseudocolorMapForData
  ...
  public boolean setLocation(SGTuple2f[] pointArray) {
    ...
    final float w = this.mGraph.getGraphRectWidth() * (float) (this.mWidthValue / (xMax - xMin));
    final float h = this.mGraph.getGraphRectHeight() * (float) (this.mHeightValue / (yMax - yMin));
  ...
  protected void updateVisibility() {
    for (int ii = 0; ii < this.mDrawingElementArray.length; ii++) {
      ...
      if (Float.isInfinite(x) || Float.isNaN(x) || Float.isInfinite(y) || Float.isNaN(y)) {
      ...
      if (this.isFocused() && this.mGraph.isSymbolsVisibleAroundFocusedObjects()) {
        ...
        if (num <= MAX_NUMBER_OF_ANCHORS) {
          for (int ii = 0; ii < num; ii++) {
            ...
            pos.setLocation(
                rect.getX() + rect.getWidth() / 2.0, rect.getY() + rect.getHeight() / 2.0);
  ...
  Value getZValue(final int x, final int y, final SGTuple2f[] pointArray) {
    ...
    if (this.isGridMode()) {
      ...
      for (int ii = 0; ii < this.mDrawingElementArray.length; ii++) {
        PseudocolorMapRectangleInGraph el =
            (PseudocolorMapRectangleInGraph) this.mDrawingElementArray[ii];
  ...
  String getToolTipTextSpatiallyVaried(final int x, final int y, final SGTuple2f[] pointsArray) {
    String text = null;
    if (this.mDrawingElementArray != null) {
      for (int ii = 0; ii < this.mDrawingElementArray.length; ii++) {
        PseudocolorMapRectangleInGraph el =
  ...
  SGIIndex getIndexAt(final int x, final int y, final SGTuple2f[] pointsArray) {
    if (this.mDrawingElementArray != null) {
      for (int ii = 0; ii < this.mDrawingElementArray.length; ii++) {
        PseudocolorMapRectangleInGraph el =
            (PseudocolorMapRectangleInGraph) this.mDrawingElementArray[ii];
  ...
  protected class PseudocolorMapRectangleInGraph extends PseudocolorMapRectangle {
    ...
    boolean contains(final int x, final int y, SGTuple2f[] pointsArray) {
      SGElementGroupPseudocolorMapInGraph colorMap =
          (SGElementGroupPseudocolorMapInGraph) this.mGroup;
