...
public class SGFigureElementAxis extends SGFigureElement2D
  ...
  /** A drawing element pressed by the mouse such as a scale number or a tick mark. */
  ...
  private void init() {
    ...
    this.createXYAxisGroups(
        ...
        DEFAULT_HORIZONTAL_AXIS_TITLE,
        DEFAULT_VERTICAL_AXIS_TITLE);
   ...
   *
   * @param element figure element as an event source
  ...
  private SGValueRange calcRange(final double min, final double max, final double factor) {
  ...
  private void createXYAxisGroups(
      final SGTuple2d xRange, final SGTuple2d yRange, final String xTitle, final String yTitle) {
    // bottom
  ...
  private SGFigureAxis createAxisGroup(
      final SGTuple2d range, final int location, final String title, final boolean visible) {
  ...
  private boolean setupAxes(SGData[] dataArray) {
    ...
    if (this.mStartFlag) {
      ...
      final String xTitle = this.createTitleString(xt, AXIS_HORIZONTAL_1);
      final String yTitle = this.createTitleString(yt, AXIS_VERTICAL_1);
      ...
      Map<SGData, List<SGDataAxisInfo>> axisInfoListMap =
          new HashMap<SGData, List<SGDataAxisInfo>>();
      for (int ii = 0; ii < dataArray.length; ii++) {
        ...
        SGDataAxisInfo axisInfoHorizontal =
            new SGDataAxisInfo(
                dataArray[ii], groupHorizontal1.mAxis, xBounds, xTitle, AXIS_HORIZONTAL_1);
        SGDataAxisInfo axisInfoVertical =
            new SGDataAxisInfo(
                dataArray[ii], groupVertical1.mAxis, yBounds, yTitle, AXIS_VERTICAL_1);
  ...
  public boolean getMarginAroundGraphRect(
    ...
    // groups
    final SGAxisElement groupTop = this.getAxisGroup(AXIS_HORIZONTAL_2);
    final SGAxisElement groupBottom = this.getAxisGroup(AXIS_HORIZONTAL_1);
    final SGAxisElement groupLeft = this.getAxisGroup(AXIS_VERTICAL_1);
    final SGAxisElement groupRight = this.getAxisGroup(AXIS_VERTICAL_2);
    ...
    final float bottom =
        (float) ((rectAll.getY() + rectAll.getHeight()) - (gRect.getY() + gRect.getHeight()));
    final float left = (float) (gRect.getX() - rectAll.getX());
    final float right =
        (float) ((rectAll.getX() + rectAll.getWidth()) - (gRect.getX() + gRect.getWidth()));
  ...
  public double getValue(final int loc, final int x, final int y) {
    ...
    } else if (loc == AXIS_VERTICAL_1 || loc == AXIS_VERTICAL_2) {
  ...
  public List<SGAxis> getHorizontalAxisList() {
    final List<SGAxis> list = new ArrayList<SGAxis>();
    for (int ii = 0; ii < this.mElementsGroupList.size(); ii++) {
      final SGFigureAxis group = (SGFigureAxis) this.mElementsGroupList.get(ii);
  ...
  public List<SGIPropertyDialogObserver> getSelectedPropertyDialogObserverList(Class<?> cl) {
    ...
    if (SGFigureAxis.class.equals(cl)) {
      ...
      if (this.mZAxisElementsGroup.isVisible() && this.mZAxisElementsGroup.isSelected()) {
  ...
  private boolean setOlderProperties(final Element element) {
    ...
    if (SGUtility.isVersionNumberEqualOrSmallerThanPermittingEmptyString(versionNumber, "2.0.0")) {
      ...
      } else if (titleFontStyleStr.length() != 0 && scaleFontStyleStr.length() != 0) {
      ...
      } else if (titleFontSizeStr.length() != 0 && scaleFontSizeStr.length() != 0) {
  ...
  public boolean fitAxisRangeToFocusedData(
    ...
    return this.fitAxisRangeToData(element, dList, axisDirection, forAnimationFrames);
  }
  ...
  public boolean fitAxisRangeToData(
    ...
    Map<SGData, SGISXYAxisShiftEnabled> mapDataToShift =
        new HashMap<SGData, SGISXYAxisShiftEnabled>();
    ...
    for (int ii = 0; ii < dList.size(); ii++) {
      ...
      List<SGDataAxisInfo> axisInfoList = element.getAxisInfoList(data, forAnimationFrames);
      axisInfoListMap.put(data, axisInfoList);
    }
  ...
  private FitAxisResult fitAxes(
    ...
    for (int ii = 0; ii < gList.size(); ii++) {
      ...
      if (allSXY) {
        if (axisDirection == AXIS_DIRECTION_HORIZONTAL && this.isHorizontal(axis)) {
          range = this.calcRange(min, max, 0.0);
        } else if (axisDirection == AXIS_DIRECTION_VERTICAL && this.isVertical(axis)) {
      ...
      if (axis.isValidValue(min) && axis.isValidValue(max)) {
        ...
        if (rangeNew.equals(rangeCur) == false) {
          if (axisDirection == AXIS_DIRECTION_NORMAL) {
            if (this.isColorBarAvailable() && axis.equals(this.mZAxisElementsGroup.mAxis)) {
          ...
          for (int jj = 0; jj < infoList.size(); jj++) {
            ...
            if (info.getAxis().equals(group.mAxis)) {
              final boolean visible =
                  (this.isHorizontal(group.mAxis) != tickLabelHorizontal.booleanValue());
  ...
  private void synchronizeDataColumnChange(
    ...
    if (SGIFigureElement.NOTIFY_DATA_STRUCTURE_CHANGE_ON_PREVIEW.equals(msg)
      ...
      if (dList.size() == 1) {
        ...
        throw new Error(
            "Data column selection for multiple data is not supported. dList.size=" + dList.size());
      }
  ...
  public SGPropertyResults setProperties(SGPropertyMap map) {
    ...
    while (itr.hasNext()) {
      ...
      if (COM_AXIS_FRAME_VISIBLE.equalsIgnoreCase(key)) {
        ...
        if (b == null) {
          result.putResult(COM_AXIS_FRAME_VISIBLE, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        ...
        result.putResult(COM_AXIS_FRAME_LINE_WIDTH, SGPropertyResults.SUCCEEDED);
        ...
        result.putResult(COM_AXIS_FRAME_COLOR, SGPropertyResults.SUCCEEDED);
      }
