...
public class SGSpinner extends JSpinner
  ...
  public SGSpinner(
      final SpinnerNumberModel model, final String unit, final int min, final int max) {
    super();
    this.initProperties(model, unit, min, max);
  }

  /**
   * Initialize the spinner with fraction digits. This method must be called immediately on the
   * creation of this spinner instance.
   *
   * @param model Number model of this spinner. Note that the range of this model is modified
   *     appropriately.
  ...
  public void initProperties(
    ...
    final double minValue =
        SGUtilityNumber.roundOffNumber(((Number) model.getMinimum()).doubleValue(), round);
    final double maxValue =
        SGUtilityNumber.roundOffNumber(((Number) model.getMaximum()).doubleValue(), round);
  ...
  /**
   * An overridden method of JSpinner class. Commits the value to the <code>SpinnerModel</code>
   * after rounding up the value.
   */
        ...
        new Runnable() {
          public void run() {
            ...
            if (mTempString != null && str.equals(mTempString) == false) {
              String unit = getUnit();
              if (isInteger() == false) {
                String strOld = SGUtilityText.removeSuffix(mTempString, unit);
   ...
   *
   * @param mode the commitment mode
   */
  ...
  /**
   * Get suffix from the current txt. If spaces between the value and the suffix, they are attached
   * to the head of the returned value.
   */
