...
public abstract class SGElementGroupBarForData extends SGElementGroupBar
  ...
  protected SGPropertyResults setProperties(SGPropertyMap map, SGPropertyResults iResult) {
    ...
    while (itr.hasNext()) {
      ...
      if (COM_DATA_BAR_BASELINE_VALUE.equalsIgnoreCase(key)) {
        ...
        if (cl == null) {
          result.putResult(
              COM_DATA_BAR_BODY_GRADATION_COLOR1, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        ...
        if (cl == null) {
          result.putResult(
              COM_DATA_BAR_BODY_GRADATION_COLOR2, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        ...
        if (index == null) {
          result.putResult(
              COM_DATA_BAR_BODY_GRADATION_DIRECTION, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        ...
        if (index == null) {
          result.putResult(
              COM_DATA_BAR_BODY_GRADATION_ORDER, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
  ...
  protected boolean getProperties(SGPropertyMap map) {
    SGPropertyUtility.addProperty(map, COM_DATA_BAR_VISIBLE, this.isVisible());
    SGPropertyUtility.addProperty(map, COM_DATA_BAR_BASELINE_VALUE, this.getBaselineValue());
    SGPropertyUtility.addProperty(map, COM_DATA_BAR_BODY_WIDTH_VALUE, this.getWidthValue());
    String[] barKeys = {
    ...
    SGPropertyUtility.addProperty(map, COM_DATA_BAR_OFFSET_X, this.getOffsetX());
    SGPropertyUtility.addProperty(map, COM_DATA_BAR_OFFSET_Y, this.getOffsetY());
    return true;
  }
}
