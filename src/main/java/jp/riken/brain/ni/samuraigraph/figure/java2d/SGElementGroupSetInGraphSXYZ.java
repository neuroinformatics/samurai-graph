...
public class SGElementGroupSetInGraphSXYZ extends SGElementGroupSetInGraph
  ...
  public boolean updateDrawingElementsLocation(final SGIData data) {
    ...
    if (this.isGridMode((SGData) data)) {
      ...
      // set the location of the data points
      if (this.mGraph.calcLocationOfPoints(
              xyValueArray, this.getXAxis(), this.getYAxis(), this.mPointsArray)
          == false) {
        return false;
      }
      ...
      for (int ii = 0; ii < gList.size(); ii++) {
        ...
        if (group.isVisible()) {
          if (group instanceof SGElementGroupPseudocolorMap) {
            SGElementGroupPseudocolorMapInGraph colorMap =
                (SGElementGroupPseudocolorMapInGraph) group;
  ...
  public boolean hasValidRectangleWidthValue(final int config, final Number value) {
    final SGAxis axis =
        (config == -1) ? this.mXAxis : this.mGraph.getAxisElement().getAxisInPlane(config);
    final double v = (value != null) ? value.doubleValue() : this.getRectangleWidthValue();
    return axis.isValidValue(v);
  }

  public boolean hasValidRectangleHeightValue(final int config, final Number value) {
    final SGAxis axis =
  ...
  protected SGPropertyResults setProperties(SGPropertyMap map, SGPropertyResults iResult) {
    ...
    for (int ii = 0; ii < colorMaps.size(); ii++) {
      SGElementGroupPseudocolorMapForData colorMap =
          (SGElementGroupPseudocolorMapForData) colorMaps.get(ii);
  ...
  public SGPropertyMap getCommandPropertyMap(SGExportParameter params) {
    ...
    // drawing elements
    SGElementGroupPseudocolorMapForData colorMapGroup =
        (SGElementGroupPseudocolorMapForData) this.getColorMapGroup();
    colorMapGroup.getProperties(map);

    return map;
  }

  private final Map<SGDataViewerDialog, List<SGTuple2f>> mFocusedSymbolSizeListMap =
      new HashMap<SGDataViewerDialog, List<SGTuple2f>>();
  ...
  private FocusSymbolResult calcFocusSymbolX(int[] xIndices, double[] xValues, float[] widthArray) {
  ...
  private FocusSymbolResult calcFocusSymbolY(
      int[] yIndices, double[] yValues, float[] heightArray) {
  ...
  protected void paintFocusedShapes(Graphics2D g2d) {
    ...
    Iterator<Entry<SGDataViewerDialog, List<SGTuple2d>>> itr =
        this.mFocusedValueListMap.entrySet().iterator();
    while (itr.hasNext()) {
  ...
  protected String getToolTipText(final int x, final int y) {
    ...
    SGElementGroupPseudocolorMapInGraph colorMap =
        (SGElementGroupPseudocolorMapInGraph) this.getColorMap();
    ...
    if (dataValue != null) {
      ...
      if (!dataValue.zValue.missing) {
        if (Double.isNaN(dataValue.zValue.number)) {
          ...
          zValue = SGUtilityNumber.getNumberInRangeOrder(dataValue.zValue.number, this.mZAxis);
        }
      } else {
        zValue = dataValue.zValue.number;
      }
