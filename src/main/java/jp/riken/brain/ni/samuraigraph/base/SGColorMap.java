...
public abstract class SGColorMap implements Cloneable, SGIDisposable {
  ...
  /**
   * Sets the value range. The minimum value corresponds to the first element and the maximum value
   * corresponds to the last element respectively of given color array.
   *
   ...
   */
  public void setValueRange(final double min, final double max, final int scaleType) {
  ...
  public Color evaluate(final double value, final int scaleType) {
    ...
    final double ratioAll = (valueInScale - minValueInScale) / (maxValueInScale - minValueInScale);
  ...
  /**
   * Returns the corresponding color for a given value. The input value should be in a scale that
   * the minimum value and the maximum value in the current scale is given by 0.0 and 1.0,
   * respectively.
   *
   * @param value a value
  ...
  public SGPropertyResults setProperties(SGPropertyMap map, SGPropertyResults iResult) {
    ...
    while (itr.hasNext()) {
      ...
      if (COM_COLOR_MAP_REVERSED.equalsIgnoreCase(key)) {
        ...
        if (b == null) {
          result.putResult(COM_COLOR_MAP_REVERSED, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        this.setReversedOrder(b.booleanValue());
        result.putResult(COM_COLOR_MAP_REVERSED, SGPropertyResults.SUCCEEDED);
      }
    }
    return result;
  }
