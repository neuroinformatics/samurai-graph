...
public abstract class SGFigureElement implements SGIFigureElement {
  ...
  /**
   * Clear focused objects. By default, this method clear all focused objects if an origin is not
   * equal to this.
   *
   ...
   *
   * @param command a command to notify
   */
  ...
  /**
   * Calculate the coordinate of a point for a given value with a given axis for given direction.
   *
   * @param value the value for a given axis
   * @param axis an axis
   * @param horizontal true if a given axis is horizontal
   */
  public float calcLocation(final double value, final SGAxis axis, final boolean horizontal) {
   ...
   *
   * @param pos the location of a point
   * @param axis an axis
   * @param horizontal true if a given axis is horizontal
   */
  public double calcValue(final float pos, final SGAxis axis, final boolean horizontal) {
  ...
  protected SGAxisValue calcBaselineValue(SGAxis axis) {
    ...
    if (axis.getDateMode()) {
      ...
      int pMillis = (int) (rangeMillis % 1000L);
      long temp = rangeMillis / 1000L;
      int pSeconds = (int) (temp % 60L);
      temp /= 60L;
      int pMinutes = (int) (temp % 60L);
      temp /= 60L;
      int pHours = (int) (temp % 24L);
      temp /= 24L;
      int pDays = (int) (temp % 30L);
      temp /= 30L;
      int pMonths = (int) (temp % 12L);
      temp /= 12L;
      int pYears = (int) temp;

      SGPeriod p =
          SGPeriod.years(pYears)
              .plusMonths(pMonths)
              .plusDays(pDays)
              .plusHours(pHours)
              .plusMinutes(pMinutes)
              .plusSeconds(pSeconds)
              .plusMillis(pMillis);
      ...
      ZonedDateTime baselineDateTime = ZonedDateTime.of(1970, 1, 1, 0, 0, 0, 0, ZoneId.of("UTC"));
      if (p.getYears() != 0) {
  ...
  protected MouseDragResult getMouseDragResult(MouseEvent e) {
    MouseDragInput params =
        new MouseDragInput(e, this.mPressedPoint, this.mDraggedDirection, this.mFixedCoordinate);
   ...
   * @param el a drawing element of text to edit
   * @param tx x-coordinate of the clicked point of the text field
   * @param ty y-coordinate of the clicked point of the text field
   */
  protected void showEditField(
      final JTextField tf, SGDrawingElementString el, final int tx, final int ty) {
   ...
   *
   * @param tf the text field
   * @param font font in the text field
   */
  protected void updateTextField(final JTextField tf, final Font font) {
    ...
    Rectangle2D stringRect = font.getStringBounds(str, new FontRenderContext(null, false, false));
