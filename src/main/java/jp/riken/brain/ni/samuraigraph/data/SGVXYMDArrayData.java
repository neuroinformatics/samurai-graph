...
public class SGVXYMDArrayData extends SGTwoDimensionalMDArrayData implements SGIVXYTypeData {
   ...
   * @param firstInfo information of the first component values
   * @param secondInfo information of the second component values
   * @param isPolar true for polar coordinate
   * @param xStride stride for x values
   * @param yStride stride for y values
    ...
    if (scatterStride != null) {
      ...
      if (yLen != xLen) {
        throw new IllegalArgumentException(
            "Length of dimension for y values is invalid: " + yLen + " != " + xLen);
      }
      if (fLen != xLen) {
        throw new IllegalArgumentException(
            "Length of dimension for the first component is invalid: " + fLen + " != " + xLen);
      }
      if (sLen != xLen) {
        throw new IllegalArgumentException(
            "Length of dimension for the second component is invalid: " + sLen + " != " + xLen);
      }
      ...
      if (firstXDim == null) {
        throw new IllegalArgumentException(
            "Failed to get the dimension for x values of the first component.");
      }
      final Integer firstYDim = firstInfo.getDimensionIndex(KEY_VXY_Y_DIMENSION);
      if (firstYDim == null) {
        throw new IllegalArgumentException(
            "Failed to get the dimension for y values of the second component.");
      }
      ...
      if (xInfo != null) {
        ...
        if (firstDims[firstXDim] != xLen) {
          throw new IllegalArgumentException(
              "Length of dimension for x values of the first component is invalid: "
                  + firstDims[firstXDim]
                  + " != "
                  + xLen);
        }
        if (secondDims[secondXDim] != xLen) {
          throw new IllegalArgumentException(
              "Length of dimension for x values of the second component is invalid: "
                  + secondDims[secondXDim]
                  + " != "
                  + xLen);
        }
      }
      if (yInfo != null) {
        ...
        if (firstDims[firstYDim] != yLen) {
          throw new IllegalArgumentException(
              "Length of dimension for y values of the first component is invalid: "
                  + firstDims[firstYDim]
                  + " != "
                  + yLen);
        }
        if (secondDims[secondYDim] != yLen) {
          throw new IllegalArgumentException(
              "Length of dimension for y values of the second component is invalid: "
                  + firstDims[secondYDim]
                  + " != "
                  + yLen);
        }
      }
  ...
  public double[] getFirstComponentValueArray(final boolean all, final boolean useCache) {
    double[] ret =
        this.getValueArray(
            this.mFirstComponentVariable, all, KEY_VXY_X_DIMENSION, KEY_VXY_Y_DIMENSION);
    if (useCache) {
  ...
  public double[] getSecondComponentValueArray(final boolean all, final boolean useCache) {
    double[] ret =
        this.getValueArray(
            this.mSecondComponentVariable, all, KEY_VXY_X_DIMENSION, KEY_VXY_Y_DIMENSION);
    if (useCache) {
  ...
  public boolean writeProperty(Element el, final SGExportParameter params) {
    ...
    if (SGIConstants.OPERATION.SAVE_TO_PROPERTY_FILE.equals(type)
      ...
      el.setAttribute(
          SGIDataPropertyKeyConstants.KEY_SECOND_COMPONENT_VARIABLE_NAME, sb.toString());
      ...
      el.setAttribute(
          ...
          this.mFirstComponentVariable.getName());
      el.setAttribute(
          SGIDataPropertyKeyConstants.KEY_SECOND_COMPONENT_VARIABLE_NAME,
          this.mSecondComponentVariable.getName());
    }
  ...
  public List<SGXYSimpleDoubleValueIndexBlock> getFirstComponentValueBlockListSub(
    ...
    Integer xIndex = this.mFirstComponentVariable.getDimensionIndex(KEY_VXY_X_DIMENSION);
    Integer yIndex = this.mFirstComponentVariable.getDimensionIndex(KEY_VXY_Y_DIMENSION);
    List<SGXYSimpleDoubleValueIndexBlock> ret =
        this.get2DValueBlockList(
            this.mFirstComponentVariable, xIndex, yIndex, this.mXStride, this.mYStride, all);
    if (useCache) {
  ...
  public List<SGXYSimpleDoubleValueIndexBlock> getSecondComponentValueBlockListSub(
    ...
    List<SGXYSimpleDoubleValueIndexBlock> ret =
        this.get2DValueBlockList(
            this.mSecondComponentVariable, xIndex, yIndex, this.mXStride, this.mYStride, all);
    if (useCache) {
  ...
  protected double[] getValueArraySub(
      final boolean all,
      final SGMDArrayVariable var,
  ...
  private VXYExportInfo exportCommon(SGExportParameter mode, SGDataBufferPolicy policy) {
    ...
    info.fDataType =
        (this.mFirstComponentVariable != null) ? this.mFirstComponentVariable.getDataType() : null;
    info.sDataType =
        (this.mSecondComponentVariable != null)
            ? this.mSecondComponentVariable.getDataType()
            : null;
    info.xValid = xValid;
    info.yValid = yValid;
