...
//	/** The linkage object required for JDirect 3 on MacOS X. */
//	private static Object linkage;

//	/** The framework to reference on MacOS X */
...
//	private static final String NETSCAPE_OPEN_PARAMETER_END = ")'"; //$NON-NLS-1$
	 ...
	 */
	private SGBrowserLauncher() { /* nothing */
	}
		...
		case MRJ_2_0:
			try {
				...
				Class appleEventClass = Class
						.forName("com.apple.MacOS.AppleEvent"); //$NON-NLS-1$
		...
		case MRJ_2_1:
			try {
				...
				Field systemFolderField = mrjFileUtilsClass
						.getDeclaredField("kSystemFolderType"); //$NON-NLS-1$
				kSystemFolderType = systemFolderField.get(null);
				findFolder = mrjFileUtilsClass.getDeclaredMethod(
						"findFolder", new Class[] { mrjOSTypeClass }); //$NON-NLS-1$
				getFileCreator = mrjFileUtilsClass.getDeclaredMethod(
						"getFileCreator", new Class[] { File.class }); //$NON-NLS-1$
				getFileType = mrjFileUtilsClass.getDeclaredMethod(
						"getFileType", new Class[] { File.class }); //$NON-NLS-1$
			} catch (ClassNotFoundException cnfe) {
				errorMessage = cnfe.getMessage();
				return false;
			} catch (NoSuchFieldException nsfe) {
...
//						.newInstance(new Object[] { SGBrowserLauncher.class });
				constructor.newInstance(new Object[] { SGBrowserLauncher.class });
			} catch (ClassNotFoundException cnfe) {
				errorMessage = cnfe.getMessage();
				return false;
			} catch (NoSuchMethodException nsme) {
		...
		case MRJ_3_1:
			try {
				...
				openURL = mrjFileUtilsClass.getDeclaredMethod(
						"openURL", new Class[] { String.class }); //$NON-NLS-1$
			} catch (ClassNotFoundException cnfe) {
				errorMessage = cnfe.getMessage();
				return false;
			} catch (NoSuchMethodException nsme) {
	...
	private static Object locateBrowser() {
		...
		case MRJ_2_1:
			...
			try {
				...
				errorMessage = ite.getTargetException().getClass()
						+ ": " + ite.getTargetException().getMessage(); //$NON-NLS-1$
				return browser;
			}
		...
		default:
			// browser = "netscape";
			String[] _browsers = { "firefox", "opera", "konqueror", "epiphany", "mozilla", "netscape" };
			browser = null;
			for ( int cnt = 0; cnt < _browsers.length && browser == null; cnt++ ) {
			  try {
			    if ( Runtime.getRuntime().exec( new String[] { "which", _browsers[cnt]} ).waitFor() == 0 )
			      browser = _browsers[cnt];	
			    } catch ( IOException e ) {
	...
	public static void openURL(String url) throws IOException {
		if (!loadedWithoutErrors) {
			throw new IOException(
					"Exception in finding browser: " + errorMessage); //$NON-NLS-1$
		}
		Object _browser = locateBrowser();
		if (_browser == null) {
			throw new IOException("Unable to locate browser: " + errorMessage); //$NON-NLS-1$
		}
		switch (jvm) {
		case MRJ_2_0:
			Object aeDesc = null;
			try {
				...
				putParameter.invoke(_browser, new Object[] { keyDirectObject,
						aeDesc });
				sendNoReply.invoke(_browser, new Object[] {});
			} catch (InvocationTargetException ite) {
				throw new IOException(
						"InvocationTargetException while creating AEDesc: " + ite.getMessage()); //$NON-NLS-1$
			} catch (IllegalAccessException iae) {
				throw new IOException(
						"IllegalAccessException while building AppleEvent: " + iae.getMessage()); //$NON-NLS-1$
			} catch (InstantiationException ie) {
				throw new IOException(
						"InstantiationException while creating AEDesc: " + ie.getMessage()); //$NON-NLS-1$
			} finally {
		...
		case MRJ_3_0:
			...
			if (result == 0) {
				...
				if (result == 0) {
					...
					throw new IOException("Unable to launch URL: " + result); //$NON-NLS-1$
				}
			} else {
				throw new IOException(
						"Unable to create an Internet Config instance: " + result); //$NON-NLS-1$
			}
			break;
		case MRJ_3_1:
			try {
				...
				throw new IOException(
						"InvocationTargetException while calling openURL: " + ite.getMessage()); //$NON-NLS-1$
			} catch (IllegalAccessException iae) {
				throw new IOException(
						"IllegalAccessException while calling openURL: " + iae.getMessage()); //$NON-NLS-1$
			}
			break;
		case WINDOWS_NT:
		case WINDOWS_9x:
			...
			Process process = Runtime.getRuntime().exec(
					new String[] { (String) _browser, FIRST_WINDOWS_PARAMETER,
			...
			try {
				...
				throw new IOException(
						"InterruptedException while launching browser: " + ie.getMessage()); //$NON-NLS-1$
			}
			break;
		case OTHER:
			...
			process = Runtime.getRuntime().exec(
					new String[] { (String) _browser, url });
