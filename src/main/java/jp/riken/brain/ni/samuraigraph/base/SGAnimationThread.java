...
public class SGAnimationThread implements Runnable, SGIDisposable {
  ...
  public SGAnimationThread(SGIAnimation[] animations) {
    ...
    SGIntegerSeriesSet indices =
        (indicesCommon != null) ? indicesCommon : SGIntegerSeriesSet.createInstance(frameNumber);
   ...
   *
   * @param b true to set enabled the loop play back
   */
   ...
   *
   * @param t sleeping time in units of milliseconds
   */
   ...
   *
   * @param b true to set playing forward
   */
   ...
   *
   * @param indices the array of available frame indices
   */
   ...
   *
   * @param index the frame index tried to be set
   * @param frameNumber the total number of frames
   * @param indices the array of available frame indices
   * @param forward true for forward and false for backward
   * @param loopPlayback true for loop play back
   * @return valid index if found, otherwise -1
   */
  public static int getValidFrameIndex(
      final int index,
      final int frameNumber,
      final int[] indices,
      final boolean forward,
      final boolean loopPlayback) {
    if (indices.length == 0) {
      return -1;
    }
    int indexNew = index;
  ...
  public void setFrameIndices(final SGIntegerSeriesSet indices) {
    ...
    for (SGIAnimation animation : this.mAnimations) {
      animation.setFrameIndices((SGIntegerSeriesSet) indices.clone());
    }
  }
}
