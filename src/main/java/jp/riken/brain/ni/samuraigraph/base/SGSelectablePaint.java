...
public class SGSelectablePaint extends SGTransparentPaint {
  ...
  public static final int[] STYLE_INDEXES = {
    STYLE_INDEX_FILL, STYLE_INDEX_PATTERN, STYLE_INDEX_GRADATION
  };

  public static final String[] STYLE_NAMES = {
    STYLE_NAME_FILL, STYLE_NAME_PATTERN, STYLE_NAME_GRADATION
  };
  ...
  public boolean getProperties(SGPropertyMap map, Map<COMMAND_KEYS, String> keyMap) {
    ...
    SGPropertyUtility.addProperty(
        map,
        keyMap.get(COMMAND_KEYS.PAINT_STYLE),
        SGSelectablePaint.getStyleName(this.getSelectedStyle()));

    // fill paint
    SGPropertyUtility.addProperty(map, keyMap.get(COMMAND_KEYS.FILL_COLOR), this.getFillColor());

    // pattern color
    SGPropertyUtility.addProperty(
        map, keyMap.get(COMMAND_KEYS.PATTERN_COLOR), this.getPatternColor());

    // pattern type
    final int patternIndex = this.getPatternIndex();
    String patternName = SGPatternPaint.getTypeName(patternIndex);
    SGPropertyUtility.addProperty(map, keyMap.get(COMMAND_KEYS.PATTERN_TYPE), patternName);

    // colors
    Color[] colors = this.getGradationPaint().getColors();
    SGPropertyUtility.addProperty(map, keyMap.get(COMMAND_KEYS.GRADATION_COLOR1), colors[0]);
    SGPropertyUtility.addProperty(map, keyMap.get(COMMAND_KEYS.GRADATION_COLOR2), colors[1]);
    ...
    SGPropertyUtility.addProperty(map, keyMap.get(COMMAND_KEYS.GRADATION_DIRECTION), directionName);

    // order
    final int orderIndex = this.getGradationPaint().getOrderIndex();
    String orderName = SGGradationPaint.getOrderName(orderIndex);
    SGPropertyUtility.addProperty(map, keyMap.get(COMMAND_KEYS.GRADATION_ORDER), orderName);

    SGTransparentPaint transPaint = (SGTransparentPaint) this;
    SGPropertyUtility.addProperty(
        map, keyMap.get(COMMAND_KEYS.TRANSPARENCY), transPaint.getTransparencyPercent());
  ...
  public SGPropertyMap getPropertyFileMap() {
    ...
    SGPropertyUtility.addProperty(
        map, KEY_PAINT_STYLE, SGSelectablePaint.getStyleName(this.getSelectedStyle()));
    SGPropertyUtility.addProperty(
        map, KEY_FILL_COLOR, SGUtilityText.getColorString(this.getFillColor()));
    SGPropertyUtility.addProperty(
        map, KEY_PATTERN_COLOR, SGUtilityText.getColorString(this.getPatternColor()));
    SGPropertyUtility.addProperty(
        map, KEY_PATTERN_TYPE, SGPatternPaint.getTypeName(this.getPatternIndex()));
