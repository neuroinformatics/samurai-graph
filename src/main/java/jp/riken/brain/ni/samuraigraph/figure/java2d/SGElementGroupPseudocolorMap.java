...
public abstract class SGElementGroupPseudocolorMap extends SGElementGroupRectangle
  ...
  private void updateSizeSub(
      SGIntegerSeries series, final float[] coordinateArray, final float[] sizeArray) {
      ...
      if (indices.length == 1) {
        ...
        if (cur == 0) {
          ...
          if (ii == 0) {
            ...
            final int prev = indices[ii - 1];
            final int next = indices[ii + 1];
            final float diffPrev = Math.abs(coordinateArray[cur] - coordinateArray[prev]);
            final float diffNext = Math.abs(coordinateArray[next] - coordinateArray[cur]);
  ...
  protected static class PseudocolorMapRectangle extends RectInGroup {
   ...
   *
   * @param values values for colors
   */
  public void setColors(final double[] values) {
    ...
    if (values.length != this.mDrawingElementArray.length) {
      throw new IllegalArgumentException(
          "Illegal array length: " + values.length + " != " + this.mDrawingElementArray.length);
    }
   ...
   *
   * @param zValues an array of z-values
   * @return true if succeeded
   */
   ...
   *
   * @param blockList the list of arrays of integer series
   * @return true if succeeded
   */
   ...
   * Sets the grid mode.
   *
   * @param b true to set the grid mode
   * @return true if succeeded
   */
