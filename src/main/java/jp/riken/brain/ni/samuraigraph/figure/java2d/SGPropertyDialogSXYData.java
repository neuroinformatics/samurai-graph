...
public class SGPropertyDialogSXYData extends SGDataDialog
  ...
  private boolean initProperty() {
    ...
    this.mErrorBarSymbolSizeSpinner.setDescription("Error Bar-> Symbol-> Size");
    this.mErrorBarLineWidthSpinner.setDescription("Error Bar-> Symbol-> Line Width");
    this.mTickLabelFontSizeSpinner.setDescription("Tick Label-> Font-> Size");
    this.mTickLabelAngleSpinner.setDescription("Tick Label-> Text-> Angle");
    this.mTickLabelDecimalPlacesSpinner.setDescription("Tick Label-> Format-> Decimal Places");
    this.mTickLabelExponentSpinner.setDescription("Tick Label-> Format-> Exponent");
  ...
  public void stateChanged(final ChangeEvent e) {
    ...
    if (source == this.mGradationPaintDialog) {
      ...
      if (null != this.getTransparency()) {
        this.setBarBodyTransparentComponentEnabled(
            Integer.valueOf(this.getTransparency().intValue()));
      }
    } else if (source == this.mSymbolBodyTransparencySpinner) {
      if (null != this.getSymbolInnerTransparency()) {
        this.setSymbolBodyTransparentComponentEnabled(
            Integer.valueOf(this.getSymbolInnerTransparency().intValue()));
      }
    }
  ...
  public boolean setTickLabelFontStyle(final Object obj) {
    ...
    this.mTickLabelFontStyleComboBox.setSelectedItem(
        SGUtilityText.getFontStyleName(style.intValue()));
    return true;
  }
      ...
      if (source.equals(this.mLineEditButton)) {
        ...
        if (!this.mPatternPaintDialog.isVisible()) {
          this.mPatternPaintDialog.setSelectedPatternPaint(
              this.mBarBodyColorPatternPaintButton.getPatternPaint());
        ...
        if (!this.mGradationPaintDialog.isVisible()) {
          this.mGradationPaintDialog.setSelectedGradationPaint(
              this.mBarBodyColorGradationColorButton.getGradationPaint());
  ...
  private void updateLineStyle(SGISXYDataDialogObserver obs, SGDataColumnInfo[] cols) {
    ...
    if (this.mLineColorAutoAssigned) {
      lineStyleList =
          SGUtilityForFigureElement.createLineStyleList(obs.getLineColorMap(), childNum);
    } else {
    ...
    for (int ii = 0; ii < lineStyleList.size(); ii++) {
      ...
      if (lineTypeItem != null) {
        final int lineType = SGDrawingElementLine.getLineTypeFromName(lineTypeItem.toString());
  ...
  private void updateLineStyleComponents(List<SGLineStyle> lineStyleList) {
    ...
    final boolean available =
        (this.mPropertyDialogObserverList.size() == 1) && (lineStyleList.size() > 1);
  ...
  private void setupBarVerticalByChangingNetCDFDataColumn(SGDataColumnInfo[] colInfo, SGData data) {
    ...
    if (SGDataUtility.isPickupColumnContained(colInfo)) {
      ...
      Map<SGDataColumnInfo, SGDataColumnInfo> lNameMap =
          new HashMap<SGDataColumnInfo, SGDataColumnInfo>();
      Map<SGDataColumnInfo, SGDataColumnInfo> uNameMap =
          new HashMap<SGDataColumnInfo, SGDataColumnInfo>();
  ...
  private void setupBarVerticalByChangingTextDataColumn(SGDataColumnInfo[] colInfo, SGData data) {
    ...
    if (null != dataSXY.isTickLabelHorizontal()) {
      ...
      if (!directionChanged) {
        if (!Boolean.valueOf(tickLabelAlignedHorizontally)
            .equals(dataSXY.isTickLabelHorizontal())) {
          directionChanged = true;
        }
      }
    }
  ...
  private boolean getBarIntervalEnabledInMultipleSeriesBars() {
    SGIDataPropertyDialogObserver obs =
  ...
  */
  ...
  private boolean setBarTabProperties(final List<SGIPropertyDialogObserver> list) {
    ...
    this.mBarIntervalComponentGroup.setEnabled(this.getBarIntervalEnabledInMultipleSeriesBars());
  ...
  private boolean setErrorBarTabProperties(final List<SGIPropertyDialogObserver> list) {
    ...
    for (int ii = 0; ii < list.size(); ii++) {
      SGISXYDataDialogObserver g = (SGISXYDataDialogObserver) list.get(ii);
  ...
  private boolean setTickLabelTabProperties(final List<SGIPropertyDialogObserver> list) {
    ...
    final boolean dateTickLabelEnabled =
        (dateTickLabel != null) ? dateTickLabel.booleanValue() : false;
  ...
  private boolean setErrorBarProperties(SGISXYDataDialogObserver l) {
    ...
    final Integer type = SGDrawingElementErrorBar.getHeadTypeFromName(headSymbolTypeName);
    if (type != null) {
  ...
  */
  ...
  protected void showLineStyleDialog() {
    ...
    this.mLineStyleDialog.setLocation(this.getX() + 20, this.getY() + 20);
  ...
  public void onXAxisDateSelected(boolean selected) {
    ...
    this.setBarDateComponentsEnabled(selected, this.mDateYComponentGroup.isEnabled());
  }

  @Override
  public void onYAxisDateSelected(boolean selected) {
    this.mDateYComponentGroup.setEnabled(selected);
    this.setBarDateComponentsEnabled(this.mDateXComponentGroup.isEnabled(), selected);
  }

  private void setBarDateComponentsEnabled(final boolean xAxisDate, final boolean yAxisDate) {
    Boolean barVertical = this.isBarVertical();
