...
public class SGArrowUtility implements SGIVXYDataConstants {
  ...
  public static boolean setLineWidth(
      final float lw, final String unit, final SGElementGroupArrow arrows) {
   ...
   * @return true if succeeded
   */
  public static boolean setHeadSize(
      final float size, final String unit, final SGElementGroupArrow arrows) {
    final Float sNew =
   ...
   * @return true if succeeded
   */
  public static boolean setHeadAngle(
      final Float openAngle, final Float closeAngle, final SGElementGroupArrow arrows) {
    final Float oNew =
        (openAngle != null)
            ? SGUtility.calcPropertyValue(
                openAngle,
                null,
                null,
                ARROW_HEAD_OPEN_ANGLE_MIN,
                ARROW_HEAD_OPEN_ANGLE_MAX,
                ARROW_HEAD_ANGLE_MINIMAL_ORDER)
            : Float.valueOf(arrows.mHeadOpenAngle);
    if (oNew == null) {
      return false;
    }
    final Float cNew =
        (closeAngle != null)
            ? SGUtility.calcPropertyValue(
                ...
                null,
                null,
                ARROW_HEAD_CLOSE_ANGLE_MIN,
                ARROW_HEAD_CLOSE_ANGLE_MAX,
                ARROW_HEAD_ANGLE_MINIMAL_ORDER)
            : Float.valueOf(arrows.mHeadCloseAngle);
    if (cNew == null) {
      return false;
    }
    if (cNew <= oNew) {
  ...
  protected static SGPropertyResults setProperties(
      SGPropertyMap map, SGPropertyResults iResult, SGElementGroupArrow arrows) {
    ...
    while (itr.hasNext()) {
      ...
      if (SGIDataCommandConstants.COM_DATA_ARROW_LINE_WIDTH.equalsIgnoreCase(key)) {
        ...
        result.putResult(SGIDataCommandConstants.COM_DATA_ARROW_COLOR, SGPropertyResults.SUCCEEDED);
  ...
  */
  ...
  static double calcXYRatio(
    ...
    if (!invariant) {
      ...
      xyRatio = (graph.getGraphRectHeight() / yRange) / (graph.getGraphRectWidth() / xRange);
    }
    return xyRatio;
  }
  ...
  private static SGTuple2f calcOrthogonalEnd(
    ...
    if (invariant) {
      ...
      final double magnitude = Math.sqrt(xComponent * xComponent + yComponent * yComponent);
      double angle;
  ...
  private static SGTuple2f calcPolarEnd(
    ...
    if (invariant) {
      ...
      final double yComponent = magnitude * Math.sin(angle);
          ...
          getOrthogonalEnd(startX, startY, xComponent, yComponent, magnitude, factor, xyRatio));
    }
    return end;
  }

  private static SGTuple2f getOrthogonalEnd(
      final float startX,
      final float startY,
      final double xComponent,
      final double yComponent,
  ...
  private static SGTuple2f getPolarEnd(
      final float startX,
      final float startY,
      final double magnitude,
      final double angle,
  ...
  static boolean calcEndLocation(
      SGIVXYTypeData dataVXY,
      SGFigureElementGraph graph,
      SGElementGroupSetInGraph groupSet,
      final float magPerCM,
      final boolean invariant,
      double[] firstValueArray,
      double[] secondValueArray,
    ...
    if (polar) {
      ...
      for (int ii = 0; ii < startArray.length; ii++) {
        if (Double.isNaN(magnitudeArray[ii])
            || Double.isNaN(angleArray[ii])
  ...
  static SGTuple2f calcEndLocation(
      final float startX,
      final float startY,
      final double fValue,
      final double sValue,
    ...
    if (Double.isNaN(startX)
      ...
      if (polar) {
        ...
        if (magnitude < 0.0) {
          ...
          endPoint.setValues(
              SGArrowUtility.calcPolarEnd(
                  startX, startY, fValue, sValue, invariant, factor, xyRatio));
        }
      } else {
        endPoint.setValues(
            SGArrowUtility.calcOrthogonalEnd(
                startX, startY, fValue, sValue, invariant, factor, xyRatio));
      }
    }
    return endPoint;
