...
public class SGUtilityForFigureElementJava2D {
   ...
   */
  public static void drawAnchorAsFocusedObject(final Point2D pos, final Graphics2D g2d) {
   ...
   * @param g2d graphic context
   */
  public static void drawAnchorAsChildObject(final List<Point2D> pointList, final Graphics2D g2d) {
    for (int ii = 0; ii < pointList.size(); ii++) {
      Point2D pos = (Point2D) pointList.get(ii);
   ...
   * @return
   */
  public static boolean clipGraphRect(final SGFigureElement el, final Graphics2D g2d) {
  ...
  static SGPropertyResults checkDataElementGroupVisibility(
      SGIElementGroupSetXY obs, SGPropertyMap map, SGPropertyResults iResult) {
    ...
    boolean symbolVisibleNew =
        (symbolVisible != null) ? symbolVisible.booleanValue() : curSymbolVisible;
  ...
  static double calcSizeValue(final double[] valueArray, SGAxis axis, final boolean horizontal) {
  ...
  public static void drawCircle(
    ...
    Shape circle = new Ellipse2D.Float(x - 0.5f * size, y - 0.5f * size, size, size);
  ...
  public static void drawSquare(
    ...
    final float x = (float) pos.getX();
    final float y = (float) pos.getY();
    Shape circle = new Rectangle2D.Float(x - 0.5f * size, y - 0.5f * size, size, size);
    drawSymbol(circle, lineWidth, lineColor, color, g2d);
  }
  ...
  /**
   * Modifies an input text string for the compatibility with previous releases older than 2_0_0.
   *
  ...
  public static Map<String, SGProperties> getElementGroupPropertiesMap(SGIElementGroupSetXY gs) {
    ...
    if (gs.isTickLabelAvailable()) {
      pMap.put(
          SGIElementGroupSetXY.ELEMENT_GROUP_TICK_LABEL, gs.getTickLabelGroup().getProperties());
    }
    return pMap;
  }
  ...
  public static Map<String, SGProperties> getElementGroupPropertiesMap(SGIElementGroupSetSXYZ gs) {
    Map<String, SGProperties> pMap = new HashMap<String, SGProperties>();
  ...
  public static Map<String, SGProperties> getElementGroupPropertiesMap(SGIElementGroupSetVXY gs) {
    Map<String, SGProperties> pMap = new HashMap<String, SGProperties>();
