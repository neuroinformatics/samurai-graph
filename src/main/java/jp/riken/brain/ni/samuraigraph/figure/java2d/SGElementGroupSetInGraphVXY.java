...
public class SGElementGroupSetInGraphVXY extends SGElementGroupSetInGraph
  ...
  /** The array of x-coordinates. These values are used only in the grid mode. */
  protected float[] mXArray = null;

  /** The array of y-coordinates. These values are used only in the grid mode. */
   ...
   *
   * @param data the data
   * @param graph the graph that this object belongs.
   */
  ...
  /**
   * Sets whether the direction is invariant when the scale of the x-axis direction and the y-axis
  ...
  public boolean updateDrawingElementsLocation(final SGIData data) {
    ...
    if (this.isGridMode((SGData) data)) {
      ...
      if (this.mGraph.calcLocationOfPoints(xValues, this.getXAxis(), true, this.mXArray) == false) {
        return false;
      }
      if (this.mGraph.calcLocationOfPoints(yValues, this.getYAxis(), false, this.mYArray)
          == false) {
        return false;
      }

      List<SGXYSimpleDoubleValueIndexBlock> firstBlockList =
          dataVXY.getFirstComponentValueBlockList();
      if (firstBlockList == null) {
        return false;
      }
      List<SGXYSimpleDoubleValueIndexBlock> secondBlockList =
          dataVXY.getSecondComponentValueBlockList();
      ...
      final double xyRatio =
          SGArrowUtility.calcXYRatio(dataVXY, this.mGraph, this, this.isDirectionInvariant());
      ...
      if (this.mGraph.calcLocationOfPoints(xyValueArray, xAxis, yAxis, this.mStartPointsArray)
          == false) {
        return false;
      }
      ...
      if (SGArrowUtility.calcEndLocation(
              dataVXY,
              this.mGraph,
              this,
              this.getMagnitudePerCM(),
  ...
  public void paintGraphics2D(final Graphics2D g2d) {
    ...
    for (int ii = 0; ii < list.size(); ii++) {
      final SGElementGroupArrowInGraph group = (SGElementGroupArrowInGraph) list.get(ii);
      if (group.isVisible()) {
  ...
  public boolean writeProperty(final Element el, final SGExportParameter params) {
    ...
    el.setAttribute(SGIFigureElementGraph.KEY_POLAR, Boolean.toString(polar));

    // write magnification
    final float factor = this.getMagnitudePerCM();
    el.setAttribute(SGIFigureElementGraph.KEY_MAGNITUDE_PER_CM, Float.toString(factor));

    final boolean invariant = this.isDirectionInvariant();
    el.setAttribute(SGIFigureElementGraph.KEY_DIRECTION_INVARIANT, Boolean.toString(invariant));
  ...
  protected SGPropertyResults setProperties(SGPropertyMap map, SGPropertyResults iResult) {
    ...
    while (itr.hasNext()) {
      ...
      if (SGIDataCommandConstants.COM_DATA_ARROW_MAGNITUDE_PER_CM.equalsIgnoreCase(key)) {
        Float mag = SGUtilityText.getFloat(value);
        if (mag == null) {
          result.putResult(SGIDataCommandConstants.COM_DATA_ARROW_MAGNITUDE_PER_CM, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        ...
        result.putResult(SGIDataCommandConstants.COM_DATA_ARROW_MAGNITUDE_PER_CM, SGPropertyResults.SUCCEEDED);
      } else if (SGIDataCommandConstants.COM_DATA_ARROW_DIRECTION_INVARIANT.equalsIgnoreCase(key)) {
        Boolean b = SGUtilityText.getBoolean(value);
        if (b == null) {
          result.putResult(
              SGIDataCommandConstants.COM_DATA_ARROW_DIRECTION_INVARIANT, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        ...
        result.putResult(SGIDataCommandConstants.COM_DATA_ARROW_DIRECTION_INVARIANT, SGPropertyResults.SUCCEEDED);
      } else if (SGIDataCommandConstants.COM_DATA_X_ARRAY_SECTION.equalsIgnoreCase(key)) {
  ...
  public SGPropertyMap getCommandPropertyMap(SGExportParameter params) {
    ...
    SGPropertyUtility.addProperty(map, SGIDataCommandConstants.COM_DATA_POLAR, Boolean.toString(vxyData.isPolar()));
    SGPropertyUtility.addProperty(map, SGIDataCommandConstants.COM_DATA_ARROW_MAGNITUDE_PER_CM, this.getMagnitudePerCM());
    SGPropertyUtility.addProperty(
        map, SGIDataCommandConstants.COM_DATA_ARROW_DIRECTION_INVARIANT, this.isDirectionInvariant());
  ...
  protected String getToolTipText(int x, int y) {
    ...
    SGIIndex index = arrowGroup.getIndexAt(x, y, this.mStartPointsArray, this.mEndPointsArray);
    if (index == null) {
      return null;
    }
    ...
    if (this.isGridMode()) {
      ...
      dataValue = arrowGroup.getDataValueAt(dataVXY, tdIndex.getColumn(), tdIndex.getRow());
    } else {
    ...
    if (dataValue != null) {
      final double xValue = SGUtilityNumber.getNumberInRangeOrder(dataValue.xValue, this.mXAxis);
      final double yValue = SGUtilityNumber.getNumberInRangeOrder(dataValue.yValue, this.mYAxis);
      final double fValue;
    ...
    String spatiallyVariedText =
        arrowGroup.getToolTipTextSpatiallyVaried(
            x, y, this.mStartPointsArray, this.mEndPointsArray);
    String notSpatiallyVariedText = data.getToolTipTextNotSpatiallyVaried();

    return this.createToolTipText(spatiallyVariedText, notSpatiallyVariedText, dataValueText);
  }

  @Override
  public String getAxisValueString(final int x, final int y) {
    SGElementGroupArrowInGraph arrowGroup = (SGElementGroupArrowInGraph) this.getArrowGroup();
    VXYDataValue componentValues =
        arrowGroup.getComponentValues(x, y, this.mStartPointsArray, this.mEndPointsArray);
