...
public class SGHDF5Variable extends SGMDArrayVariable {
   ...
   * Builds an variable.
   *
   * @param file the file of HDF5 data
   * @param fqName the fully qualified name of a data set
   */
  ...
  public double getDoubleValue(
    ...
    return this.getDoubleValue(mdArray, dimensionIndex, arrayIndex, origins.clone());
  }
   ...
   *
   * @param dimensionIndex dimension index
   * @param stride stride of the array
   * @param origins the origins
  ...
  public double[] getDoubleArray(
    ...
    try {
      ...
      for (int ii = 0; ii < array.length; ii++) {
        array[ii] = this.getDoubleValue(mdArray, dimensionIndex, indices[ii], origins.clone());
      }
    } catch (Exception e) {
      if (e.getClass().getName().endsWith("HDF5DatatypeInterfaceException")) {
        try {
          if (HDF5DataClass.INTEGER.equals(this.getDataClass())) {
            ...
            for (int ii = 0; ii < array.length; ii++) {
              array[ii] =
                  (double) this.getLongValue(mdArray, dimensionIndex, indices[ii], origins.clone());
            }
          }
        } catch (Exception e2) {
  ...
  private double getDoubleValue(
      MDDoubleArray mdArray, final int dimensionIndex, final int arrayIndex, final int[] origins) {
    if (dimensionIndex < 0 || dimensionIndex >= origins.length) {
      throw new IllegalArgumentException("Index out of bounds: " + dimensionIndex);
    }
    origins[dimensionIndex] = arrayIndex;
  ...
  private long getLongValue(
      MDLongArray mdArray, final int dimensionIndex, final int arrayIndex, final int[] origins) {
    if (dimensionIndex < 0 || dimensionIndex >= origins.length) {
      throw new IllegalArgumentException("Index out of bounds: " + dimensionIndex);
    }
    origins[dimensionIndex] = arrayIndex;
  ...
  /**
   * Returns the array of double values for given dimension indices of x and y values with given
   * origins.
   *
  ...
  /**
   * Returns the array of double values for given dimension indices with given stride and given
   * origins.
   *
   * @param xIndex dimension index for x-values
   * @param yIndex dimension index for y-values
   * @param xSeries array indices for x-values
   * @param ySeries array indices for y-values
   * @param origins the origins
   * @return the array of double values
   */
  @Override
  public double[] getDoubleArray(
      ...
      SGIntegerSeries ySeries,
      final int[] origins) {
    return this.getDoubleArray(
        xIndex, yIndex, new SGIntegerSeriesSet(xSeries), new SGIntegerSeriesSet(ySeries), origins);
  }
  ...
  public String[] getStringArray(
      final int dimensionIndex, SGIntegerSeriesSet stride, int[] origins) {
    int[] dims = this.getDimensions();
    if (dimensionIndex < 0 || dimensionIndex >= dims.length) {
      throw new IllegalArgumentException("Index out of bounds: " + dimensionIndex);
    }
    ...
    if (HDF5DataClass.STRING.equals(dClass)) {
      try {
        ...
        for (int ii = 0; ii < array.length; ii++) {
          array[ii] = this.getString(mdArray, dimensionIndex, indices[ii], origins.clone());
        }
      } catch (Exception e) {
        if (e.getClass().getName().endsWith("HDF5DatatypeInterfaceException")) {
          if (e instanceof RuntimeException) {
   ...
   */
  @Override
  public String getString(final int dimensionIndex, final int arrayIndex, final int[] origins) {
    int[] dims = this.getDimensions();
    if (dimensionIndex < 0 || dimensionIndex >= dims.length) {
      throw new IllegalArgumentException("Index out of bounds: " + dimensionIndex);
    }
    MDArray<String> mdArray = this.readStringMDArray();
    return this.getString(mdArray, dimensionIndex, arrayIndex, origins.clone());
  }
  ...
  protected static class HDF5DataType extends MDArrayDataType {
    protected HDF5DataType(HDF5DataClass dataClass) {
      ...
      if (HDF5DataClass.INTEGER.equals(dataClass) || dataClass == null) {
