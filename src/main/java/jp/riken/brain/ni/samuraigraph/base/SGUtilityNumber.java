...
public class SGUtilityNumber implements SGIConstants {
  ...
  /**
   * Truncate the number at given digit. For example: value=8715.61, digit=2 -> 8700.0
   *
  ...
  /**
   * Round off the number at given digit. For example: value=8715.61, digit=1 -> 8700.0 and
   * value=8765.61, digit=1 -> 8800.0
   *
  ...
  /**
   * Round out the number at given digit. For example: value=8715.61, digit=1 -> 8800.0
   *
   ...
   *
   * @param start1 the start value of the first range
   * @param end1 the end value of the first range
   * @param start2 the start value of the second range
   * @param end2 the end value of the second range
   ...
   * @return
   */
  public static boolean contains(final double x1, final double x2, final double value) {
    double min;
    double max;
    if (x1 <= x2) {
      min = x1;
  ...
  /**
   * Returns a rounded number that is the order of given number to given significant digits with
   * given mode of rounding.
   *
   * @param value the number to be rounded
   * @param num a number to specify the order
   * @param digit significant digits
   * @param mode the mode of rounding
   * @return a rounded number
   */
  public static double getNumberInNumberOrder(
      final double value, final double num, final int digit, final int mode) {
    if (value == 0.0) {
      return value;
    }
  ...
  /**
   * Returns a rounded number that is the order of given value range to given significant digits
   * with given mode of rounding.
   *
   * @param value the number to be rounded
  ...
  public static double getNumberInRangeOrder(
      final double value, final double min, final double max, final int digit, final int mode) {
    if (max == min) {
   ...
   */
  public static double[] calcStepValueSorted(
      final double min,
      final double max,
  ...
  public static double getNumberInRangeOrder(
      final double value, final SGAxis axis, final int digit, final int mode) {
    final int type = axis.getScaleType();
    if (type == SGAxis.LINEAR_SCALE) {
      return SGUtilityNumber.getNumberInRangeOrder(
          value, axis.getMinDoubleValue(), axis.getMaxDoubleValue(), digit, mode);
      ...
      if (value == 0.0) {
        ...
        return SGUtilityNumber.getNumberInRangeOrder(value, min, max, digit, mode);
      }
    } else {
  ...
  public static double getNumberInRangeOrder(
      final double value, final double[] values, final int digit) {
    final double min = SGUtilityNumber.min(values);
    final double max = SGUtilityNumber.max(values);
    return SGUtilityNumber.getNumberInRangeOrder(
        value, min, max, digit, RoundingMode.HALF_UP.ordinal());
  }
   ...
   */
  private static String createExponentialString(
      final double value, final NumberFormat nf, final int exp) {
