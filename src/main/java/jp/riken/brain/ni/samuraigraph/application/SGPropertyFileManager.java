...
class SGPropertyFileManager
  ...
  public int savePropertiesByDialog(final SGDrawingWindow wnd) {
    ...
    String name = SGApplicationUtility.appendExtension(fileName, PROPERTY_FILE_EXTENSION);
  ...
  boolean getInfoFromPropertyFile(
      final File file,
      final Component parent,
      final List<SGPropertyFileData> pDataList,
  ...
  boolean getInfoFromPropertyFile(
    ...
    for (int ii = 0; ii < fnLength; ii++) {
      ...
      NodeList dataList = figure.getElementsByTagName(SGIFigureElementGraph.TAG_NAME_DATA);
      final int id = ii + 1;
  ...
  private boolean fromMultiDataFileChooser(final ActionEvent e) {
    ...
    if (command.equals(SGDialog.OK_BUTTON_TEXT)) {
      ...
      // set properties
      if (!this.setPropertyFileFromEventDispatchThread(
          wnd, this.mPropertyFileDocument, wDataArray)) {
        return false;
      }
  ...
  private void createAllWizardDialogsToLoadPropertyFromToolBar(final SGDrawingWindow owner) {
    ...
    // set the selected file name
    this.mMultiDataFileChooserWizardDialog.setSelectedFile(
        dir, this.mMain.getCurrentFileName(FILE_TYPE.TXT_DATA));
  ...
  boolean showMultiDataFileChooserDialog(final File propertyFile, final Frame owner) {
    ...
    if (createFlag) {
      ...
      this.mMultiDataFileChooserWizardDialogDD =
          new SGPropertyDataFileChooserWizardDialog(owner, true);
      ...
      this.mMultiDataFileChooserWizardDialogDD.setSelectedFile(
          this.mMain.getCurrentFileDirectory(), this.mMain.getCurrentFileName(FILE_TYPE.TXT_DATA));
    }
