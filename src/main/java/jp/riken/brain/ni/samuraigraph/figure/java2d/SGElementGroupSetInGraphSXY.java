...
public class SGElementGroupSetInGraphSXY extends SGElementGroupSetInGraph
  ...
  public boolean hasValidBaselineValue(final int config, final Number value) {
    final SGAxis axis =
        (config == -1) ? this.mYAxis : this.mGraph.getAxisElement().getAxisInPlane(config);
  ...
  public SGElementGroupErrorBar createErrorBars(SGISXYTypeSingleData dataSXY) {
    final SGElementGroupErrorBarInGraph group =
        new SGElementGroupErrorBarInGraph(dataSXY, this.mGraph);
    this.mDrawingElementGroupList.add(group);
  ...
  public boolean updateDrawingElementsLocation(final SGIData data) {
    ...
    if (barGroup.isVisible() || (errorBarGroup.isVisible() && !errorBarGroup.isPositionOnLine())) {
  ...
  public Rectangle2D getTickLabelsBoundingBox(final SGIData data) {
    ...
    final SGElementGroupTickLabel group = tickLabelGroups.get(0);
    if (!group.isVisible()) {
      return null;
    }
  ...
  public void paintGraphics2D(final Graphics2D g2d) {
    ...
    // line
    final SGElementGroupLineInGraph groupLine = this.mGraph.getGroupLine(this);
    if (groupLine != null) {
    ...
    // symbol
    final SGElementGroupSymbolInGraph groupSymbol = this.mGraph.getGroupSymbol(this);
    if (groupSymbol != null) {
  ...
  public SGElementGroup getElementGroupAt(final int x, final int y) {
    ...
    SGElementGroupErrorBarInGraph errorBarGroup =
        (SGElementGroupErrorBarInGraph) this.getErrorBarGroup();
    SGElementGroupTickLabelInGraph tickLabelGroup =
        (SGElementGroupTickLabelInGraph) this.getTickLabelGroup();
  ...
  public static class SXYElementGroupSetPropertiesInFigureElement
    ...
    public boolean equals(final Object obj) {
      ...
      SXYElementGroupSetPropertiesInFigureElement p =
          (SXYElementGroupSetPropertiesInFigureElement) obj;
  ...
  public void setFocusedValueIndices(
      final SGDataViewerDialog dg, int[] focusedIndices, int[] arraySectionIndices) {
  ...
  protected void setFocusedValueIndices(
      SGDataViewerDialog dg, List<Integer> indexList, int[] arraySectionIndices) {
    // refresh the map
    ...
    if (lineSymbolVisible) {
      for (int pointIndex : pointIndexList) {
        SGTuple2d value =
            this.calcLineValue(this.mXValuesArray[pointIndex], this.mYValuesArray[pointIndex]);
    ...
    if (this.isBarVisible()) {
      ...
      for (int pointIndex : pointIndexList) {
        ...
        SGTuple2d value =
            this.calcBarValue(this.mXValuesArray[pointIndex], this.mYValuesArray[pointIndex]);
        if (bar.isVertical()) {
  ...
  protected String getToolTipText(final int x, final int y) {
    ...
    if (graphRect.contains(x, y)) {
      ...
      if (symbolGroup.isVisible()) {
        indexList = symbolGroup.getIndicesAt(x, y, linePoints, xValueArray, yValueArray);
        if (indexList != null) {
      ...
      SGTuple2f[] barPoints = null;
      if (barGroup.isVisible()
          || (this.isErrorBarAvailable()
              && errorBarGroup.isVisible()
      ...
      if (this.isErrorBarAvailable()) {
        SGTuple2f[] errorBarPoints = errorBarGroup.isPositionOnLine() ? linePoints : barPoints;
        if (errorBarGroup.isVisible()) {
          indexList = errorBarGroup.getIndicesAt(x, y, errorBarPoints, xValueArray, yValueArray);
          if (indexList != null) {
      ...
      if (barGroup.isVisible()) {
        indexList = barGroup.getIndicesAt(x, y, barPoints, xValueArray, yValueArray);
        if (indexList != null) {
      ...
      if (lineGroup.isVisible()) {
        indexList = lineGroup.getIndicesAt(x, y, linePoints, xValueArray, yValueArray);
        if (indexList != null) {
  ...
  private SXYSingleDataValue getShiftedValue(SGISXYTypeSingleData data, SXYSingleDataValue value) {
  ...
  private String getDataValueTooltipText(final int index0, final int index1) {
    ...
    final double xValue0 = SGUtilityNumber.getNumberInRangeOrder(value0.xValue, this.mXAxis);
  ...
  private String getToolTipTextReturned(final String spatiallyVariedText, List<Integer> indices) {
    ...
    return this.createToolTipText(
        sbSpatiallyVaried.toString(), notSpatiallyVariedText, dataValueText);
  }
  ...
  protected List<Integer> getDataViewerRowIndex(final int x, final int y) {
    ...
    if (graphRect.contains(x, y)) {
      ...
      if (symbolGroup.isVisible()) {
        ret = symbolGroup.getIndicesAt(x, y, linePoints, xValueArray, yValueArray);
        if (ret != null) {
          return ret;
        }
      }
      ...
      if (this.isErrorBarAvailable()) {
        SGTuple2f[] errorBarPoints = errorBarGroup.isPositionOnLine() ? linePoints : barPoints;
        if (errorBarGroup.isVisible()) {
          ret = errorBarGroup.getIndicesAt(x, y, errorBarPoints, xValueArray, yValueArray);
          if (ret != null) {
            return ret;
          }
        }
      ...
      if (barGroup.isVisible()) {
        ret = barGroup.getIndicesAt(x, y, barPoints, xValueArray, yValueArray);
        if (ret != null) {
          return ret;
        }
      }
      ...
      if (lineGroup.isVisible()) {
        ret = lineGroup.getIndicesAt(x, y, linePoints, xValueArray, yValueArray);
        if (ret != null) {
          return ret;
        }
      }
