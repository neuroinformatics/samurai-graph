...
public abstract class SGFigureElementForData extends SGFigureElement2D
  ...
  private HashMap<String, SGPropertyDialog> mDataDialogMap =
      new HashMap<String, SGPropertyDialog>();
  ...
  protected SGIElementGroupSetForData createGroupSet(
    ...
    if (data instanceof SGISXYTypeSingleData) {
      SGIElementGroupSetSXY groupSetSXY =
          this.createGroupSetSXY((SGISXYTypeSingleData) data, axisX, axisY, false, name);
      this.setInfoMapToGroupSet(groupSetSXY, infoMap);
      groupSet = groupSetSXY;
    } else if (data instanceof SGISXYTypeMultipleData) {
      SGIElementGroupSetMultipleSXY groupSetSXY =
          this.createGroupSetMultipleSXY((SGISXYTypeMultipleData) data, axisX, axisY, name);
      this.setInfoMapToGroupSet(groupSetSXY, infoMap);
      ...
      groupSet = this.createGroupSetVXY((SGIVXYTypeData) data, axisX, axisY, name);
    } else if (data instanceof SGISXYZTypeData) {
      SGAxis axisZ = this.mAxisElement.getZAxis();
      groupSet = this.createGroupSetSXYZ((SGISXYZTypeData) data, axisX, axisY, axisZ, name);
    } else {
  ...
  protected SGIElementGroupSetSXY createGroupSetSXY(
      ...
      final String name) {
    SGIElementGroupSetSXY groupSet = this.createSingleGroupSetSXY(data, axisX, axisY, multiple);
    if (groupSet == null) {
      return null;
    }
    groupSet.setName(name);
  ...
  protected SGIElementGroupSetSXY createSingleGroupSetSXY(
    ...
    final SGIElementGroupSetSXY groupSet = this.createSXYGroupSetInstance(data);
    if (groupSet == null) {
      return null;
    }
    ...
    if (!multiple) {
      ...
      if (data != null) {
        ...
        final double[] valueArray =
            vertical ? data.getXValueArray(false) : data.getYValueArray(false);
        final SGAxis axis = vertical ? axisX : axisY;
        final double widthValue = this.calcDefaultBarWidth(valueArray, axis);
  ...
  protected double getBarWidthOfBarSeries(SGIElementGroupSetXY gs) {
    ...
    if (bar.isVisible()) {
      if (data instanceof SGISXYTypeSingleData) {
        valueArray =
            vertical
                ? ((SGISXYTypeSingleData) data).getXValueArray(false)
                : ((SGISXYTypeSingleData) data).getYValueArray(false);
      } else if (data instanceof SGISXYTypeMultipleData) {
        valueArray =
            vertical
                ? ((SGISXYTypeMultipleData) data).getXValueArray(false)[0]
  ...
  protected SGIElementGroupSetVXY createGroupSetVXY(
      final SGIVXYTypeData data, final SGAxis axisX, final SGAxis axisY, final String name) {
    SGIElementGroupSetVXY groupSet = this.createSingleGroupSetVXY(data, axisX, axisY);
    if (groupSet == null) {
      return null;
    }
    groupSet.setName(name);
  ...
  protected SGIElementGroupSetSXYZ createGroupSetSXYZ(
      final SGISXYZTypeData data, final SGAxis axisX, final SGAxis axisY, final String name) {
    SGIElementGroupSetSXYZ groupSet = this.createSingleGroupSetSXYZ(data, axisX, axisY);
    if (groupSet == null) {
      return null;
    }
    groupSet.setName(name);
  ...
  protected SGIElementGroupSetVXY createSingleGroupSetVXY(
    ...
    final SGIElementGroupSetVXY groupSet = this.createVXYGroupSetInstance(data);
    if (groupSet == null) {
      return null;
    }
  ...
  protected SGIElementGroupSetSXYZ createSingleGroupSetSXYZ(
    ...
    final SGIElementGroupSetSXYZ groupSet = this.createSXYZGroupSetInstance(data);
    if (groupSet == null) {
      return null;
    }
  ...
  protected SGIElementGroupSetMultipleSXY createGroupSetMultipleSXY(
    ...
    final SGIElementGroupSetMultipleSXY groupSet = this.createMultipleSXYGroupSetInstance(data);
    if (groupSet == null) {
      return null;
    }
    ...
    try {
      ...
      if (gsArray.length < sxyArray.length) {
        ...
        for (int ii = gsArray.length; ii < sxyArray.length; ii++) {
          SGData d = (SGData) sxyArray[ii];
          SGIElementGroupSetSXY gs =
              this.createSingleGroupSetSXY((SGISXYTypeSingleData) d.copy(), axisX, axisY, true);
          if (gs == null) {
            return false;
          }
          gs.setID(ii + 1);
  ...
  protected SGIElementGroupSetSXYZ createGroupSetSXYZ(
      ...
      final SGAxis axisZ,
      final String name) {
    SGIElementGroupSetSXYZ groupSet = this.createSingleGroupSetSXYZ(data, axisX, axisY, axisZ);
    if (groupSet == null) {
      return null;
    }
    groupSet.setName(name);
  ...
  // create an instance of SXYZ groupSet
  protected SGIElementGroupSetSXYZ createSingleGroupSetSXYZ(
      final SGISXYZTypeData data, final SGAxis axisX, final SGAxis axisY, final SGAxis axisZ) {
  ...
  public boolean createDataObject(
    ...
    SGIElementGroupSetForData gs =
        this.createGroupSet(el, data, axisX, axisY, name, readDataProperty);
    if (gs == null) {
      return false;
    }
  ...
  protected SGIElementGroupSetForData createGroupSet(
    ...
    if (data instanceof SGISXYTypeSingleData) {
      ...
      gs = this.createGroupSetVXY((SGIVXYTypeData) data, axisX, axisY, name);
  ...
  protected int setPropertyOfElementGroupSetSXY(
    ...
    SGLineStyle lineStyle =
        new SGLineStyle(lineGroup.getLineType(), lineGroup.getColor(), lineGroup.getLineWidth());
    ...
    try {
      ...
      for (int ii = 0; ii < sxyArray.length; ii++) {
        SGIElementGroupSetSXY gs =
            (SGIElementGroupSetSXY)
                this.createGroupSet(el, (SGData) sxyArray[ii], axisX, axisY, name, false);
        if (gs == null) {
          return ic;
        }
      ...
      for (int ii = 0; ii < sxyArray.length; ii++) {
        ...
        if (gs.isErrorBarAvailable()) {
          elpMap.put(
              SGIElementGroupSetXY.ELEMENT_GROUP_ERROR_BAR, gs.getErrorBarGroup().getProperties());
        }
        if (gs.isTickLabelAvailable()) {
          elpMap.put(
              SGIElementGroupSetXY.ELEMENT_GROUP_TICK_LABEL,
          ...
          public void run() {
            if (mDataDialogMap != null) {
              if (mDataDialogMap.get(DIALOG_KEY_SXY) == null) {
                mDataDialogMap.put(DIALOG_KEY_SXY, new SGPropertyDialogSXYData(mDialogOwner, true));
              }
  ...
  public SGProperties synchronizeDataProperties(SGProperties p, SGProperties dp) {
    ...
    ElementGroupSetPropertiesInFigureElement gp =
        (ElementGroupSetPropertiesInFigureElement) p.copy();
    ...
    if (gp instanceof MultipleSXYElementGroupSetPropertiesInFigureElement) {
      MultipleSXYElementGroupSetPropertiesInFigureElement mgp =
          (MultipleSXYElementGroupSetPropertiesInFigureElement) gp;
      ElementGroupSetPropertiesInFigureElement first =
          (ElementGroupSetPropertiesInFigureElement) mgp.childPropertyList.get(0);
  ...
  private void addAxisInfo(
      final SGData data,
      final SGAxis axis,
      final SGValueRange bounds,
      final String title,
      final List<SGDataAxisInfo> aList) {
  ...
  public SGData getData(final int id) {
    ...
    SGElementGroupSetForData gs = (SGElementGroupSetForData) this.getChild(id);
    if (gs == null) {
      return null;
    }
    return gs.getData();
  ...
  public boolean addData(
    ...
    if (autoAssigned) {
      List<SGLineStyle> lineStyleList =
          SGUtilityForFigureElement.createLineStyleList(new HueColorMap(), data.length);
  ...
  public void showDataPopupMenu(
      final SGElementGroupSetForData gs,
      final int x,
      final int y,
  ...
  protected boolean synchronizeToDataElement(SGIFigureElementForData dElement, final String msg) {

    List<SGData> gDataList = dElement.getDataList();
    if (gDataList.size() != this.mDataList.size()) {
      throw new Error(
          "dataList.size()!= this.mDataList.size()  "
              + gDataList.size()
              + ", "
              + this.mDataList.size());
    }
