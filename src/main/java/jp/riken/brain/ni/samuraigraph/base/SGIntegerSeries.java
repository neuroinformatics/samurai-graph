...
public class SGIntegerSeries implements Cloneable {
  ...
  public static boolean isValidSeries(
      final SGInteger start, final SGInteger end, final SGInteger step) {
    ...
    if (nStart != null && nEnd != null && nStep != null) {
      if (!isValidSeries(nStart, nEnd, nStep)) {
        throw new IllegalArgumentException(
            "Invalid input values: " + start + ":" + step + ":" + end);
      }
    }
    this.mStart = start;
    this.mEnd = end;
   ...
   *
   * @param series an integer series
   */
  ...
  public static List<SGIntegerSeries> createList(final int[] numbers) {
    ...
    TreeMap<Integer, List<SGIntegerSeries>> rangeListMap =
        new TreeMap<Integer, List<SGIntegerSeries>>();
    int cnt = 1;
    ...
    TreeMap<Integer, List<SGIntegerSeries>> dRangeListMap =
        new TreeMap<Integer, List<SGIntegerSeries>>(rangeListMap.descendingMap());
  ...
  private static List<SGIntegerSeries> reduceOverlappingRange(
      SGIntegerSeries range, List<SGIntegerSeries> rangeList) {
  ...
  private static void updateRangeListMap(
      final int start,
      final int end,
      final int step,
      Map<Integer, List<SGIntegerSeries>> rangeListMap,
      int cnt) {
  ...
  /**
   * Creates an instance with given array length. The start is set to zero, the end is set to the
  ...
  public boolean overlap(SGIntegerSeries series) {
    return SGUtilityNumber.isOverlapping(
        this.getStart().getNumber(),
        this.getEnd().getNumber(),
        series.getStart().getNumber(),
        series.getEnd().getNumber());
  }
