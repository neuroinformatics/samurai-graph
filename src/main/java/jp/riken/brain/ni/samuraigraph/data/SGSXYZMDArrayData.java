...
public class SGSXYZMDArrayData extends SGTwoDimensionalMDArrayData implements SGISXYZTypeData {
   ...
   * @param zInfo information of z values
  ...
  public SGSXYZMDArrayData(
      ...
      final SGMDArrayDataColumnInfo zInfo,
    ...
    if (scatterStride != null) {
      ...
      if (zLen != xLen) {
        throw new IllegalArgumentException(
            "Length of dimension for x values is invalid: " + zLen + " != " + xLen);
      }
      if (zLen != yLen) {
        throw new IllegalArgumentException(
            "Length of dimension for y values is invalid: " + zLen + " != " + yLen);
      }
      ...
      if (xInfo != null) {
        ...
        if (zDims[xDim] != xLen) {
          throw new IllegalArgumentException(
              "Length of dimension for x values is invalid: " + zDims[xDim] + " != " + xLen);
        }
      }
      if (yInfo != null) {
        final int yLen = yInfo.getGenericDimensionLength();
        if (zDims[yDim] != yLen) {
          throw new IllegalArgumentException(
              "Length of dimension for y values is invalid: " + zDims[yDim] + " != " + yLen);
        }
      }
  ...
  protected SGMDArrayVariable initVariable(SGMDArrayFile file, SGMDArrayDataColumnInfo info) {
    ...
    if (info != null) {
      Integer xDim = info.getDimensionIndex(KEY_SXYZ_X_DIMENSION);
      Integer xValue = null;
      if (xDim != null) {
        xValue = xDim;
      } else {
      ...
      var.setDimensionIndex(KEY_SXYZ_X_DIMENSION, xValue);

      Integer yDim = info.getDimensionIndex(KEY_SXYZ_Y_DIMENSION);
      Integer yValue = null;
      if (yDim != null) {
        yValue = yDim;
      } else {
      ...
      var.setDimensionIndex(KEY_SXYZ_Y_DIMENSION, yValue);
  ...
  protected SGMDArrayVariable initVariable(SGMDArrayVariable var) {
    ...
    var.setDimensionIndex(KEY_SXYZ_X_DIMENSION, -1);
    var.setDimensionIndex(KEY_SXYZ_Y_DIMENSION, -1);
    return var;
  }
  ...
  public double[] getZValueArray(final boolean all, final boolean useCache) {
    double[] ret =
        this.getValueArray(this.mZVariable, all, KEY_SXYZ_X_DIMENSION, KEY_SXYZ_Y_DIMENSION);
    if (useCache) {
  ...
  public List<SGXYSimpleDoubleValueIndexBlock> getZValueBlockListSub(
    ...
    List<SGXYSimpleDoubleValueIndexBlock> ret =
        this.get2DValueBlockList(
            this.mZVariable, xIndex, yIndex, this.mXStride, this.mYStride, all);
  ...
  public Map<String, Map<String, Integer>> getUsedDimensionIndexMap() {
    ...
    dimMap.remove(KEY_GENERIC_DIMENSION);
    return map;
  }
  ...
  protected String getXDimensionKey() {
    return KEY_SXYZ_X_DIMENSION;
  }

  @Override
  protected String getYDimensionKey() {
    return KEY_SXYZ_Y_DIMENSION;
  }

  @Override
  protected String getXStrideKey() {
    return KEY_SXYZ_STRIDE_X;
  }

  @Override
  protected String getYStrideKey() {
    return KEY_SXYZ_STRIDE_Y;
  }

  @Override
  protected String getScatterStrideKey() {
    return KEY_SXYZ_INDEX_STRIDE;
  }
  ...
  public Map<String, Object> getInfoMap() {
    ...
    map.put(KEY_SXYZ_GRID_PLOT_FLAG, !this.isIndexAvailable());
    return map;
  }
    ...
    if (this.isIndexAvailable()) {
      ...
      strZ =
          this.getTwoDimensionalVarCommandString(
              this.mZVariable, KEY_SXYZ_X_DIMENSION, KEY_SXYZ_Y_DIMENSION);
    }
    varList.add(strZ);
  ...
  public String getToolTipSpatiallyVaried(final int index0, final int index1) {
    ...
    String zStr =
        this.getComponentToolTipSpatiallyVaried(
            this.mZVariable, KEY_SXYZ_X_DIMENSION, KEY_SXYZ_Y_DIMENSION, index0, index1);
    sb.append(zStr);
    return sb.toString();
  }
  ...
  @Override
  public double[] getZValueArray(boolean all, boolean useCache, boolean removeInvalidValues) {
