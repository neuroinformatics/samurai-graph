...
class SGFigureAxis extends SGAxisElement {
  ...
  protected boolean createTickMarks() {
    ...
    switch (scaleType) {
      case SGAxis.LINEAR_SCALE:
        ret =
            this.createTickMarksInLinearScale(
                this.getTickMarkWidth(), this.mMajorTickMarkLength, this.mTickMarkColor);
        break;
      case SGAxis.LOG_SCALE:
        ret =
            this.createTickMarksInLogScale(
                this.getTickMarkWidth(), this.mMajorTickMarkLength, this.mTickMarkColor);
        break;
      default:
        throw new Error("Invalid scale type: " + scaleType);
  ...
  public boolean setShift(final float shift, final String unit) {
    final Float sNew =
        SGUtility.calcPropertyValueInUnit(
            shift, unit, AXIS_SHIFT_UNIT, AXIS_SHIFT_MIN, AXIS_SHIFT_MAX, AXIS_SHIFT_MINIMAL_ORDER);
    if (sNew == null) {
      return false;
    }
  ...
  protected SGPropertyResults setProperties(SGPropertyMap map, SGPropertyResults iResult) {
    ...
    while (itr.hasNext()) {
      ...
      if (COM_AXIS_SHIFT.equalsIgnoreCase(key)) {
        ...
        result.putResult(COM_AXIS_SHIFT, SGPropertyResults.SUCCEEDED);
      } else if (COM_AXIS_DATE_MODE.equalsIgnoreCase(key)) {
        Boolean b = SGUtilityText.getBoolean(value);
        if (b == null) {
          result.putResult(COM_AXIS_DATE_MODE, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        ...
        result.putResult(COM_AXIS_DATE_MODE, SGPropertyResults.SUCCEEDED);
      } else if (COM_AXIS_DATE_FORMAT.equalsIgnoreCase(key)) {
        if (map.isDoubleQuoted(key) == false) {
          result.putResult(COM_AXIS_DATE_FORMAT, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        if (!"".equals(value)) {
        ...
        result.putResult(COM_AXIS_DATE_FORMAT, SGPropertyResults.SUCCEEDED);
  ...
  List<Point2D> getAnchorPoints() {
    ...
    if (this.isVisible()) {
      if (this.isSelected()) {
        ...
        beta = 0.75f;
        pList.add(
            new Point2D.Float(alpha * start.x + beta * end.x, alpha * start.y + beta * end.y));
        alpha = 0.75f;
        beta = 0.25f;
        pList.add(
            new Point2D.Float(alpha * start.x + beta * end.x, alpha * start.y + beta * end.y));
  ...
  public SGPropertyMap getPropertyMap() {
    ...
    SGPropertyUtility.addProperty(
        map, COM_AXIS_SHIFT, this.getShift(AXIS_SHIFT_UNIT), AXIS_SHIFT_UNIT);

    // date mode
    SGPropertyUtility.addProperty(map, COM_AXIS_DATE_MODE, this.getDateMode());

    // date format
    SGPropertyUtility.addQuotedStringProperty(
        map, COM_AXIS_DATE_FORMAT, this.getNumberDateFormat());
  ...
  public SGPropertyMap getPropertyFileMap(SGExportParameter params) {
    ...
    // shift
    final float shift = this.roundOffAxisShift(this.getShift());
    SGPropertyUtility.addProperty(map, KEY_SHIFT, Float.toString(shift) + AXIS_SHIFT_UNIT);

    // date format
    SGPropertyUtility.addProperty(map, KEY_NUMBER_DATE_FORMAT, this.getNumberDateFormat());
