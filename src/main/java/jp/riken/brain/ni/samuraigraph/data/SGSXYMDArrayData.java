...
public class SGSXYMDArrayData extends SGMDArrayData implements SGISXYTypeSingleData {
   ...
   * @param ehInfo information of the error bar holder, which must be a variable for x or y values
   * @param tlInfo information of the tick label
   * @param thInfo information of the tick label holder, which must be a variable for x or y values
  ...
  public SGSXYMDArrayData(
    ...
    if (ehInfo != null) {
      if (!ehInfo.getName().equals(xName) && !ehInfo.getName().equals(yName)) {
        throw new IllegalArgumentException(
            "Error bar holder must be the same as the x or y variable.");
      }
    }
    ...
    if (thInfo != null) {
      if (!thInfo.getName().equals(xName) && !thInfo.getName().equals(yName)) {
        throw new IllegalArgumentException(
            "Tick label holder must be the same as the x or y variable.");
      }
    }
    ...
    final int xLen =
        (xInfo != null)
            ? xInfo.getGenericDimensionLength()
            : (yInfo != null ? yInfo.getGenericDimensionLength() : -1);
    if (xLen == -1) {
      throw new IllegalArgumentException("Invalid length: " + xLen);
    }
    final int yLen =
        (yInfo != null)
            ? yInfo.getGenericDimensionLength()
            : (xInfo != null ? xInfo.getGenericDimensionLength() : -1);
    if (yLen == -1) {
    ...
    final int len =
        (xVar != null) ? xVar.getGenericDimensionLength() : yVar.getGenericDimensionLength();
  ...
  public double[] getLowerErrorValueArray(final boolean all, final boolean useCache) {
    ...
    if (all || !this.isStrideAvailable()) {
      ...
      ret = this.mLowerErrorVariable.getGenericNumberArray(this.mStride);
    }
    if (useCache) {
  ...
  public double[] getUpperErrorValueArray(final boolean all, final boolean useCache) {
    ...
    if (all || !this.isStrideAvailable()) {
      ...
      ret = this.mUpperErrorVariable.getGenericNumberArray(this.mStride);
    }
    if (useCache) {
  ...
  protected SGMDArrayDataColumnInfo getErrorBarHolderInfo(
      SGMDArrayDataColumnInfo xInfo, SGMDArrayDataColumnInfo yInfo) {
  ...
  protected SGMDArrayDataColumnInfo getTickLabelHolderInfo(
      SGMDArrayDataColumnInfo xInfo, SGMDArrayDataColumnInfo yInfo) {
   ...
   * Transforms this data to a multiple type data.
   *
   * @param pickupYVariable true to pick up the y variable, and false to pick up the x variable
   * @param pickUpDimensionIndex the dimension index to pick up
   * @return a transformed data
   */
  public SGISXYTypeMultipleData toMultiple(
    ...
    SGMDArrayDataColumnInfo t =
        bt
            ? SGDataUtility.createDataColumnInfo(
                this.mTickLabelVariable, SGIDataColumnTypeConstants.TICK_LABEL)
            : null;
    SGMDArrayDataColumnInfo pickUpInfo = pickupYVariable ? y : x;
    pickUpInfo.setDimensionIndex(
        SGIMDArrayConstants.KEY_SXY_PICKUP_DIMENSION, pickUpDimensionIndex);
  ...
  public double[] getLowerErrorValueArray(
      boolean all, boolean useCache, final boolean removeInvalidValues) {
  ...
  public void addMultipleDimensionEditedDataValue(SGDataValueHistory dataValue) {
    ...
    if (prev != null) {
      dValue =
          new SGDataValueHistory.MDArray.D1(
              mdValue.getValue(),
