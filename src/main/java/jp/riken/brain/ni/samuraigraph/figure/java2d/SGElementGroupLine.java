...
public abstract class SGElementGroupLine extends SGElementGroup {
  ...
  public static boolean writeProperty(
      final Element el, final int lineType, final Color lineColor, final float lineWidth) {
  ...
  static boolean readStyleProperty(
      final Element el, final SGElementGroupLine group, final int index) {
    String str = null;
    Number num = null;
    Color color = null;
    ...
    SGLineStyle lineStyle =
        new SGLineStyle(group.getLineType(), group.getColor(), group.getLineWidth());
   ...
   * Set the term points of each line element.
   *
   * @param pointArray original location of term points
   */
  protected boolean setLocationSub(final SGTuple2f[] pointArray) {
    ...
    if (allEffectiveFlag) {
      ...
      if (len > 1) {
        ...
        final Line2D line = new Line2D.Float(points[0].x, points[0].y, points[1].x, points[1].y);
        gp.append(line, true);
        if (len > 2) {
