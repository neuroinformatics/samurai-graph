...
class SGColorBarAxis extends SGAxisElement
  ...
  protected boolean createTickMarks() {
    ...
    switch (this.mAxis.getScaleType()) {
      case SGAxis.LINEAR_SCALE:
        flag =
            this.createTickMarksInLinearScale(lw, this.mMajorTickMarkLength, this.mTickMarkColor);
        break;
      case SGAxis.LOG_SCALE:
        flag = this.createTickMarksInLogScale(lw, this.mMajorTickMarkLength, this.mTickMarkColor);
        break;
      default:
        throw new Error();
    }
  ...
  protected void createTicksSub(
    ...
    el = this.createSingleTickMarkCounter(value, width, len, inside);
    if (el != null) {
      this.setupCreatedScaleLineProperties(el, value, cl);
    }
  }
  ...
  public Rectangle2D getColorBarRectangle() {
    ...
    Rectangle2D rect = new Rectangle2D.Double(this.getX(), this.getY(), w, h);
    return rect;
  }
  ...
  public void paintGraphics2D(final Graphics2D g2d) {
    ...
    for (int ii = 0; ii <= nDiff; ii++) {
      ...
      final Color cl = this.mColorMap.evaluate(zValue, SGAxis.LINEAR_SCALE);
      g2d.setColor(cl);
      g2d.fill(rect);
    }
    ...
    if (this.mAxisElement.isSymbolsVisibleAroundFocusedObjects() && this.isSelected()) {
  ...
  public boolean onMouseDragged(final MouseEvent e) {
    ...
    this.mColorMap.setValueRange(this.mAxis.getMinDoubleValue(), this.mAxis.getMaxDoubleValue());
    return true;
  }
  ...
  protected boolean setLocationOfTitle() {
    ...
    if (DIRECTION_HORIZONTAL_LOWER.equals(this.mDirection)
      ...
      final float x =
          this.getX() + (this.getLength() - (float) rectTitle.getWidth()) / 2.0f + shiftFromCenter;
      final float y;
      if (DIRECTION_HORIZONTAL_LOWER.equals(this.mDirection)) {
  ...
  private boolean initLocationOfExponentDrawingElement() {
    ...
    if (DIRECTION_HORIZONTAL_LOWER.equals(this.mDirection)
      ...
      x = 0.0f;
      if (DIRECTION_HORIZONTAL_LOWER.equals(this.mDirection)) {
        y = spaceLN + (float) getScaleHeight() + spaceNT;
      } else {
        y = -(spaceLN + (float) getScaleHeight() + spaceNT + (float) rect.getHeight());
      }
    } else if (DIRECTION_VERTICAL_LEFT.equals(this.mDirection)
        || DIRECTION_VERTICAL_RIGHT.equals(this.mDirection)) {
      ...
      if (DIRECTION_VERTICAL_LEFT.equals(this.mDirection)) {
        x =
            -(float)
                (spaceLN
                    + this.mAxisElement.getMaxLengthOfScaleNumbers(this)
                    + spaceNT
                    + rect.getWidth());
      } else {
   ...
   *
   * @param dx displacement to the horizontal axis direction
   * @param dy displacement to the vertical axis direction
   */
  ...
  public boolean setBarLength(final float len, final String unit) {
    final Float sNew =
        SGUtility.calcPropertyValue(
            ...
            unit,
            COLOR_BAR_SIZE_UNIT,
            COLOR_BAR_LENGTH_MIN,
            COLOR_BAR_LENGTH_MAX,
            COLOR_BAR_SIZE_MINIMAL_ORDER);
    if (sNew == null) {
      return false;
    }
  ...
  public boolean setProperties(SGProperties p) {
    ...
    Iterator<Entry<String, ColorMapProperties>> itr =
        cp.colorMapPropertiesMap.entrySet().iterator();
    while (itr.hasNext()) {
  ...
  protected boolean readProperties(final Element element, final String versionNumber) {
    ...
    // color maps
    NodeList colorMapsNodeList =
        element.getElementsByTagName(SGColorMapManager.TAG_NAME_COLOR_STYLES);
  ...
  protected SGPropertyResults setProperties(SGPropertyMap map, SGPropertyResults iResult) {
    ...
    while (itr.hasNext()) {
      ...
      if (COM_COLOR_BAR_AXIS_X.equalsIgnoreCase(key)) {
        ...
        if (loc == -1) {
          result.putResult(COM_COLOR_BAR_AXIS_X, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        ...
        result.putResult(COM_COLOR_BAR_AXIS_X, SGPropertyResults.SUCCEEDED);
      } else if (COM_COLOR_BAR_AXIS_Y.equalsIgnoreCase(key)) {
        final int loc = SGUtility.getAxisLocation(value);
        if (loc == -1) {
          result.putResult(COM_COLOR_BAR_AXIS_Y, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        ...
        result.putResult(COM_COLOR_BAR_AXIS_Y, SGPropertyResults.SUCCEEDED);
      } else if (COM_COLOR_BAR_LOCATION_X.equalsIgnoreCase(key)) {
        Double num = SGUtilityText.getDouble(value);
        if (num == null) {
          result.putResult(COM_COLOR_BAR_LOCATION_X, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        ...
        result.putResult(COM_COLOR_BAR_LOCATION_X, SGPropertyResults.SUCCEEDED);
      } else if (COM_COLOR_BAR_LOCATION_Y.equalsIgnoreCase(key)) {
        Double num = SGUtilityText.getDouble(value);
        if (num == null) {
          result.putResult(COM_COLOR_BAR_LOCATION_Y, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        if (SGUtility.isValidPropertyValue(num.doubleValue()) == false) {
          result.putResult(COM_COLOR_BAR_LOCATION_Y, SGPropertyResults.INVALID_INPUT_VALUE);
        ...
        result.putResult(COM_COLOR_BAR_LENGTH, SGPropertyResults.SUCCEEDED);
      } else if (COM_COLOR_BAR_DIRECTION.equalsIgnoreCase(key)) {
        if (this.setDirection(value) == false) {
          result.putResult(COM_COLOR_BAR_DIRECTION, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        result.putResult(COM_COLOR_BAR_DIRECTION, SGPropertyResults.SUCCEEDED);
      } else if (COM_COLOR_BAR_STYLE.equalsIgnoreCase(key)) {
        if (this.setColorBarStyle(value) == false) {
          result.putResult(COM_COLOR_BAR_STYLE, SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        result.putResult(COM_COLOR_BAR_STYLE, SGPropertyResults.SUCCEEDED);
  ...
  public SGPropertyMap getPropertyMap() {
    ...
    SGPropertyUtility.addProperty(
        map, COM_COLOR_BAR_AXIS_X, this.mAxisElement.getLocationName(this.mXAxis));
    SGPropertyUtility.addProperty(
        map, COM_COLOR_BAR_AXIS_Y, this.mAxisElement.getLocationName(this.mYAxis));
    ...
    SGPropertyUtility.addProperty(
        ...
        SGUtility.getExportValue(
            this.getBarWidth(COLOR_BAR_SIZE_UNIT), COLOR_BAR_SIZE_MINIMAL_ORDER),
        COLOR_BAR_SIZE_UNIT);
    SGPropertyUtility.addProperty(
        map,
        COM_COLOR_BAR_LENGTH,
        SGUtility.getExportValue(
            this.getBarLength(COLOR_BAR_SIZE_UNIT), COLOR_BAR_SIZE_MINIMAL_ORDER),
        COLOR_BAR_SIZE_UNIT);
  ...
  public SGPropertyMap getPropertyFileMap(SGExportParameter params) {
    ...
    // style
    SGPropertyUtility.addProperty(map, KEY_COLOR_BAR_STYLE, this.getColorBarStyle());
...
class ElementLineOfColorBarScale extends ElementLineTickMark {
  ...
  ElementLineOfColorBarScale(
      final SGTuple2f start, final SGTuple2f end, SGAxisElement axisElement) {
