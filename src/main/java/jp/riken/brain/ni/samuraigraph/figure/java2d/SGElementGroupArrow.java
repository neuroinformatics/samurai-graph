...
public abstract class SGElementGroupArrow extends SGElementGroup
  ...
  public boolean writeProperty(final Element el) {
    ...
    final float headSize =
        (float) SGUtilityNumber.roundOffNumber(this.getHeadSize() * cp, digitHeadSize);
    final float openAngle =
        (float) SGUtilityNumber.roundOffNumber(this.getHeadOpenAngle(), digitAngle);
    final float closeAngle =
        (float) SGUtilityNumber.roundOffNumber(this.getHeadCloseAngle(), digitAngle);
  ...
  protected boolean initDrawingElement(final SGTuple2f[] startArray, final SGTuple2f[] endArray) {
    ...
    for (int ii = 0; ii < bArray.length; ii++) {
      ((SGDrawingElementArrow) bArray[ii]).setLocation(startArray[ii], endArray[ii]);
    }
  ...
  /** */
  public boolean setLocation(final SGTuple2f[] startArray, final SGTuple2f[] endArray) {
    ...
    for (int ii = 0; ii < array.length; ii++) {
      final boolean eff =
          !(startArray[ii].isInfinite()
              || startArray[ii].isNaN()
              || endArray[ii].isInfinite()
              || endArray[ii].isNaN());
