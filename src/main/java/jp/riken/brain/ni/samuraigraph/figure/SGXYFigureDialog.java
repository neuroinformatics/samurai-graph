...
public class SGXYFigureDialog extends SGPropertyDialog
  ...
  public int getGridLineType() {
    final String typeName = (String) this.mGridLineTypeComboBox.getSelectedItem();
  ...
  private Boolean isDateMode(final boolean horizontal) {
    final int location =
        horizontal ? this.mAxisPanel.getXAxisLocation() : this.mAxisPanel.getYAxisLocation();
    boolean[] bArray = new boolean[this.mPropertyDialogObserverList.size()];
    for (int ii = 0; ii < this.mPropertyDialogObserverList.size(); ii++) {
      SGIXYFigureDialogObserver l =
          (SGIXYFigureDialogObserver) this.mPropertyDialogObserverList.get(ii);
  ...
  public boolean setBaseXValue(final Object value, final boolean dateMode) {
    if (dateMode) {
      return SGDateUtility.setDateValue(
          this.mBaselineXTextField, value, SGDateUtility.getUTCTimeZoneInstance());
    } else {
  ...
  public boolean setBaseYValue(final Object value, final boolean dateMode) {
    if (dateMode) {
      return SGDateUtility.setDateValue(
          this.mBaselineYTextField, value, SGDateUtility.getUTCTimeZoneInstance());
    } else {
  ...
  public boolean setDialogProperty() {
    ...
    for (int ii = 0; ii < list.size(); ii++) {
      SGIFigureDialogObserver figure = (SGIFigureDialogObserver) list.get(ii);
  ...
  private boolean setPropertiesToGridListeners(SGIXYFigureDialogObserver l) {
    ...
    try {
      ...
      if (auto != null && !auto) {
        this.addInputErrorDescription(this.mBaselineXTextField.getDescription());
    ...
    try {
      ...
      if (auto != null && !auto) {
        this.addInputErrorDescription(this.mStepXTextField.getDescription());
        validAxisValue = false;
      }
    }
    ...
    try {
      ...
      if (auto != null && !auto) {
        this.addInputErrorDescription(this.mBaselineYTextField.getDescription());
        validAxisValue = false;
      }
    }
    ...
    try {
      ...
      if (auto != null && !auto) {
        this.addInputErrorDescription(this.mStepYTextField.getDescription());
        validAxisValue = false;
      }
    }
    if (!validAxisValue) {
