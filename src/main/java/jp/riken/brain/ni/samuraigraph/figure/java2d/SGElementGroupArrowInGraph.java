...
/** The group of arrows. */
public class SGElementGroupArrowInGraph extends SGElementGroupArrowForData
   ...
   *
   * @param b a value to set the the flag whether this group is focused
   */
   ...
   *
   * @param graph the graph that this element group set belongs to
   */
  ...
  private boolean emphasisArrow(final SGDrawingElementArrow2D arrow, final Graphics2D g2d) {
    ...
    SGUtilityForFigureElementJava2D.drawAnchorAsFocusedObject(new Point2D.Float(end.x, end.y), g2d);
    return true;
  }
  ...
  VXYDataValue getComponentValues(
      final int x,
      final int y,
      final SGTuple2f[] startPointsArray,
      final SGTuple2f[] endPointsArray) {
    ...
    for (int ii = 0; ii < this.mDrawingElementArray.length; ii++) {
      ...
      if (el.contains(x, y, startPointsArray, endPointsArray)) {
        ...
        if (data instanceof SGVXYNetCDFData) {
          ...
          if (this.isGridMode()) {
            ret.fValue = new Value(ncData.getFirstComponentValueAt(el.mYIndex, el.mXIndex));
            ret.sValue = new Value(ncData.getSecondComponentValueAt(el.mYIndex, el.mXIndex));
          } else {
            ret.fValue = new Value(ncData.getFirstComponentValueAt(el.mIndex));
            ret.sValue = new Value(ncData.getSecondComponentValueAt(el.mIndex));
          }
        } else {
  ...
  protected class ArrowInGraph extends ArrowInGroup {
    ...
    boolean contains(
        final int x, final int y, SGTuple2f[] startPointsArray, SGTuple2f[] endPointsArray) {
  ...
  String getToolTipTextSpatiallyVaried(
      final int x,
      final int y,
      final SGTuple2f[] startPointsArray,
      final SGTuple2f[] endPointsArray) {
  ...
  protected boolean setLocation(
    ...
    if (firstComponentValueBlockList == null || secondComponentValueBlockList == null) {
      throw new IllegalArgumentException(
          "firstComponentValueBlockList == null || secondComponentValueBlockList == null");
    }
    if (firstComponentValueBlockList.size() != secondComponentValueBlockList.size()) {
      throw new IllegalArgumentException(
          "firstComponentValueBlockList.size() != secondComponentValueBlockList.size()");
    }
      ...
      for (int yy = 0; yy < yIndices.length; yy++) {
        ...
        for (int xx = 0; xx < xIndices.length; xx++) {
          ...
          if (Double.isNaN(fValue)
              || Double.isInfinite(fValue)
              || Double.isNaN(sValue)
              || Double.isInfinite(sValue)) {
          ...
          SGTuple2f end =
              SGArrowUtility.calcEndLocation(
                  startX, startY, fValue, sValue, polar, magPerCM, invariant, factor, xyRatio);
