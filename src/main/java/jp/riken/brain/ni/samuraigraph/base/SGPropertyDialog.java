...
public abstract class SGPropertyDialog extends SGDialog {
  ...
  protected transient List<SGIPropertyDialogObserver> mPropertyDialogObserverList =
      new ArrayList<SGIPropertyDialogObserver>();
  ...
  /**
   * Initialization method to be called in the constructor of sub classes. This method must be
   * called after the call of the constructor of the super class.
   */
  ...
  protected boolean hasValidNumber(final SGTextField tf, final Number min, final Number max) {
  ...
  protected boolean hasValidDate(final SGTextField tf, final boolean addDesc) {
    boolean valid = true;
    boolean check = false;
    if (tf.isIndeterminate()) {
      if (tf.getText() != null) {
  ...
  protected boolean hasValidPeriod(final SGTextField tf, final boolean addDesc) {
    boolean valid = true;
    boolean check = false;
    if (tf.isIndeterminate()) {
      if (tf.getText() != null) {
  ...
  private final transient List<String> mInputErrorComponentNameList = new ArrayList<String>();
  ...
  private void onPeriodButtonPressedCommon(SGPeriodInputDialog dg, SGTextField field) {
    String str = field.getText();
    Double d = SGUtilityText.getDouble(str);
    if (d == null) {
      return;
  ...
  protected void addComponentGroupMap(Component comp, Map<Component, SGComponentGroupElement> map) {
  ...
  protected void initDateFormatComboBox(final JComboBox<String> cb) {
    final String[] dateFormatArray = DEFAULT_DATE_DISPLAY_FORMAT_ARRAY;
