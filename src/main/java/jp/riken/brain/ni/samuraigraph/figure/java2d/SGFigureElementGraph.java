...
public class SGFigureElementGraph extends SGFigureElementForData implements SGIFigureElementGraph {
  ...
  private boolean updateDrawingElementsLocation(
      final SGData data, final SGElementGroupSetInGraph groupSet) {
    try {
  ...
  private boolean addData(
    ...
    SGElementGroupSetInGraph groupSet =
        (SGElementGroupSetInGraph) this.createGroupSet(data, name, infoMap);
    if (groupSet == null) {
      return false;
    }
  ...
  public boolean addData(final SGData data, final String name, final SGProperties p) {
    ...
    // create an ElementGroupSetInGraph object
    SGElementGroupSetInGraph groupSet =
        (SGElementGroupSetInGraph) this.createGroupSet(data, name, null);
    if (groupSet == null) {
      return false;
    }
  ...
  private boolean synchronizeToAxisElement(final SGIFigureElementAxis aElement, final String msg) {
    ...
    if (this.isNotificationMessage(msg)) {
      ...
      for (int ii = 0; ii < this.mChildList.size(); ii++) {
        SGElementGroupSetInGraph groupSet = (SGElementGroupSetInGraph) this.mChildList.get(ii);
   ...
   *
   * @param lElement the legend element
   * @param msg a message
   * @return true if succeeded
   */
  ...
  protected boolean isInsideAxisRange(
      final SGTuple2d value, final SGAxis axisX, final SGAxis axisY) {
   ...
   * Calculate the location of points.
   *
   * @param cArray the array of axis values
   * @param axisX the x-axis for give values
   * @param axisY the y-axis for give values
   * @param pointArray the coordinates for given values
   * @return true if succeeded
   */
  protected boolean calcLocationOfPoints(
      final SGTuple2d[] cArray,
      final SGAxis axisX,
      final SGAxis axisY,
  ...
  public boolean onMouseClicked(final MouseEvent e) {
    ...
    for (int ii = list.size() - 1; ii >= 0; ii--) {
      final SGElementGroupSetInGraph groupSet = (SGElementGroupSetInGraph) list.get(ii);
      if (groupSet.isVisible() == false) {
        continue;
      }
    ...
    for (int ii = this.mChildList.size() - 1; ii >= 0; ii--) {
      ...
      if (groupSet.onMousePressed(e)) {
        ...
        for (int jj = 0; jj < this.mChildList.size(); jj++) {
          final SGElementGroupSetInGraph gs = (SGElementGroupSetInGraph) this.mChildList.get(jj);
  ...
  public boolean createElementOfData(
    ...
    for (int ii = 0; ii < groupSetList.size(); ii++) {
      SGElementGroupSetInGraph groupSet = (SGElementGroupSetInGraph) groupSetList.get(ii);
  ...
  protected boolean setVisibleChildList(final List<SGIChildObject> list) {
    ...
    for (int ii = 0; ii < visibleList.size(); ii++) {
      SGElementGroupSetInGraph groupSet = (SGElementGroupSetInGraph) visibleList.get(ii);
  ...
  protected SGIElementGroupSetSXYZ createSXYZGroupSetInstance(SGISXYZTypeData dataSXYZ) {
    SGElementGroupSetInGraphSXYZ groupSet =
        new SGElementGroupSetInGraphSXYZ((SGData) dataSXYZ, this);
  ...
  protected SGIElementGroupSetSXY createSingleGroupSetSXY(
    ...
    SGElementGroupSetInGraphSXY groupSet =
        (SGElementGroupSetInGraphSXY) super.createSingleGroupSetSXY(data, axisX, axisY, multiple);
    if (groupSet == null) {
      return null;
    }
  ...
  protected SGIElementGroupSetVXY createSingleGroupSetVXY(
    ...
    SGElementGroupSetInGraphVXY groupSet =
        (SGElementGroupSetInGraphVXY) super.createSingleGroupSetVXY(data, axisX, axisY);
    if (groupSet == null) {
      return null;
    }
  ...
  protected SGIElementGroupSetSXYZ createSingleGroupSetSXYZ(
    ...
    SGElementGroupSetInGraphSXYZ groupSet =
        (SGElementGroupSetInGraphSXYZ) super.createSingleGroupSetSXYZ(data, axisX, axisY, axisZ);
    if (groupSet == null) {
      return null;
    }
   ...
   * @return
   */
  private Map<String, String> compareNetCDFLabelText(
  ...
  Map<SGIData, List<SGTwoDimensionalArrayIndex>> getDataTableCellListMap() {
    Map<SGIData, List<SGTwoDimensionalArrayIndex>> map =
        new HashMap<SGIData, List<SGTwoDimensionalArrayIndex>>();
    for (int ii = 0; ii < this.mChildList.size(); ii++) {
