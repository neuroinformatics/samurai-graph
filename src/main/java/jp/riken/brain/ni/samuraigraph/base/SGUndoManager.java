...
/**
 * A class to keep the property histories and to control undo/redo operation of an undoable object.
 */
public class SGUndoManager implements SGIDisposable {
  ...
  private boolean setCurrentMemento() {
    SGProperties p = this.mMementoList.get(this.mMementoCounter);
  ...
  public boolean undo() {
    ...
    List<SGIUndoable> undoableList =
        this.mChangedObjectListList.get(this.mChangedObjectListCounter - 1);
  ...
  public boolean redo() {
    ...
    List<SGIUndoable> undoableList =
        this.mChangedObjectListList.get(this.mChangedObjectListCounter);
    for (int ii = 0; ii < undoableList.size(); ii++) {
      SGIUndoable undoable = undoableList.get(ii);
      boolean flag;
      if (undoable.equals(this.mUndoable)) {
  ...
  private boolean updateObjectHistory(final List<SGIUndoable> objList) {
    List<List<SGIUndoable>> hList =
        new ArrayList<>(this.mChangedObjectListList.subList(0, this.mChangedObjectListCounter));
   ...
   *
   * @return true if the undoable object is changed
   */
  ...
  public boolean deleteForwardHistory() {
    ...
    for (int ii = this.mChangedObjectListList.size() - 1;
        ii >= this.mChangedObjectListCounter;
        ii--) {
