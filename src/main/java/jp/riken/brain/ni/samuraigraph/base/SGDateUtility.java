...
public class SGDateUtility {
   ...
   *
   * @param millis a millisecond time for date
   * @param zone time zone
   * @return a text string for date
   */
  ...
  /**
   * Sets a text string of ISO8601 format obtained from a given date value to a text field.
   *
   * @param tf a text field
   * @param obj the date value
   * @param zone time zone
  ...
  public static boolean setPeriodValue(final SGTextField tf, final Object obj) {
    if (obj == null) {
      tf.setText(null);
    } else {
      String valueStr = obj.toString().trim();
