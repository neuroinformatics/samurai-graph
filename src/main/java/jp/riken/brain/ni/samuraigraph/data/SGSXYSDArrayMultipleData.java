...
public class SGSXYSDArrayMultipleData extends SGSDArrayData
   ...
   * @param yIndices an array of the indices for y-values
   * @param lowerErrorIndices an array of the indices for lower error values
   * @param upperErrorIndices an array of the indices for upper error values
   * @param errorBarHolderIndices an array of the indices of columns those hold error bars
   * @param tickLabelIndices an array of the indices for tick labels
   * @param tickLabelHolderIndices an array of the indices of columns those hold tick labels
   */
  public SGSXYSDArrayMultipleData(
      final SGSDArrayFile dataFile,
      final SGDataSourceObserver obs,
      final Integer[] xIndices,
    ...
    if (!this.isValidErrorBarIndices(
        multipleIndices, lowerErrorIndices, upperErrorIndices, errorBarHolderIndices)) {
      throw new IllegalArgumentException("Column indices for the error bars are invalid.");
    }
    if (!this.isValidTickLabelIndices(multipleIndices, tickLabelIndices, tickLabelHolderIndices)) {
  ...
  protected boolean isValidXYValueIndices(final Integer[] xIndices, final Integer[] yIndices) {
    ...
    if (be) {
      ...
      for (int ii = 0; ii < dataArray.length; ii++) {
        ...
        for (int jj = 0; jj < this.mErrorBarHolderIndices.length; jj++) {
          final boolean b =
              by
                  ? this.mErrorBarHolderIndices[jj].equals(this.mYIndices[ii])
    ...
    if (bt) {
      ...
      for (int ii = 0; ii < dataArray.length; ii++) {
        ...
        for (int jj = 0; jj < this.mTickLabelHolderIndices.length; jj++) {
          final boolean b =
              by
                  ? this.mTickLabelHolderIndices[jj].equals(this.mYIndices[ii])
  ...
  protected boolean writeAttributeColumnIndices(Element el) {
    ...
    if (this.writeAttributeColumnIndicesSub(el, KEY_Y_VALUE_COLUMN_INDICES, this.mYIndices)
        == false) {
      return false;
    }
    if (this.mLowerErrorIndices != null
        && this.mUpperErrorIndices != null
        && this.mErrorBarHolderIndices != null) {
      if (this.writeAttributeColumnIndicesSub(
              el, KEY_LOWER_ERROR_BAR_COLUMN_INDICES, this.mLowerErrorIndices)
          == false) {
        return false;
      }
      if (this.writeAttributeColumnIndicesSub(
              el, KEY_UPPER_ERROR_BAR_COLUMN_INDICES, this.mUpperErrorIndices)
          == false) {
        return false;
      }
      if (this.writeAttributeColumnIndicesSub(
              el, KEY_ERROR_BAR_HOLDER_COLUMN_INDICES, this.mErrorBarHolderIndices)
          == false) {
        return false;
      }
    }
    if (this.mTickLabelIndices != null && this.mTickLabelHolderIndices != null) {
      ...
      if (tlIndicesList.size() != 0) {
        ...
        if (this.writeAttributeColumnIndicesSub(el, KEY_TICK_LABEL_COLUMN_INDICES, tlIndices)
            == false) {
          return false;
        }
        if (this.writeAttributeColumnIndicesSub(el, KEY_TICK_LABEL_HOLDER_COLUMN_INDICES, thIndices)
            == false) {
          return false;
        }
      }
  ...
  private boolean writeAttributeColumnIndicesSub(Element el, String key, Integer[] indices) {
  ...
  protected boolean writeSequentialColumnIndices(Element el) {
    ...
    if (this.isErrorBarAvailable()) {
      ...
      if (!this.writeAttributeColumnIndicesSub(el, KEY_UPPER_ERROR_BAR_COLUMN_INDICES, uIndices)) {
        return false;
      }
      ...
      if (!this.writeAttributeColumnIndicesSub(
          el, KEY_ERROR_BAR_HOLDER_COLUMN_INDICES, ehIndices)) {
        return false;
      }
    }
    ...
    if (this.isTickLabelAvailable() && this.hasGenericTickLabels()) {
      ...
      if (!this.writeAttributeColumnIndicesSub(el, KEY_TICK_LABEL_COLUMN_INDICES, tIndices)) {
        return false;
      }
      ...
      if (!this.writeAttributeColumnIndicesSub(
          el, KEY_TICK_LABEL_HOLDER_COLUMN_INDICES, thIndices)) {
        return false;
      }
    }
  ...
  public boolean setColumnType(String[] columns) {
    ...
    if (lList.size() != 0 && lIndices != null && uIndices != null) {
      ...
      for (int ii = 0; ii < ehIndices.length; ii++) {
        ...
        Integer eh = SGDataColumnTitleUtility.getAppendedColumnIndex(str, columnTitles);
        if (eh == null) {
          return false;
        }
        ehUeList.add(eh);
    ...
    if (tList.size() != 0 && tIndices != null) {
      ...
      for (int ii = 0; ii < thIndices.length; ii++) {
        ...
        Integer th = SGDataColumnTitleUtility.getAppendedColumnIndex(str, columnTitles);
        if (th == null) {
          return false;
        }
        thIndices[ii] = th;
        ...
        if (this.mLowerErrorIndices[ii].equals(this.mUpperErrorIndices[ii])) {
          array[this.mLowerErrorIndices[ii].intValue()] =
              SGDataColumnTitleUtility.appendColumnNoOrTitle(
                  ...
                  index,
                  this.getDataFile().isEmptyTitle(index)
                      || this.getDataFile().isRepeatedTitle(index),
                  this.getTitle(index));
        } else {
          array[this.mLowerErrorIndices[ii].intValue()] =
              SGDataColumnTitleUtility.appendColumnNoOrTitle(
  ...
  public static SGISXYTypeMultipleData merge(final List<SGData> dataList) {
    ...
    SGSXYSDArrayMultipleData dataLast =
  ...
  public Object[][] getValueTable(final SGExportParameter params, SGDataBufferPolicy policy) {
    ...
    final boolean archiveFlag = SGDataMiscUtility.isArchiveDataSetOperation(mode);
      ...
      if (archiveFlag) {
        ...
        if (xDateFlag != null && xDateFlag.booleanValue()) {
          for (int jj = 0; jj < xValues.length; jj++) {
            array[ii][cnt] = SGDataTextUtility.getTextValue(dateArray[ii].toString());
            cnt++;
          }
        } else {
          for (int jj = 0; jj < xValues.length; jj++) {
      ...
      if (tickLabels != null) {
        for (int jj = 0; jj < tickLabels.length; jj++) {
          if (tickLabels[jj] != null) {
            row[cnt] = SGDataTextUtility.getTextValue(tickLabels[jj][ii]);
            cnt++;
          }
        }
      } else {
        if (exportFlag) {
          if (xDateFlag != null) {
            ...
            row[cnt] = SGDataTextUtility.getTextValue(date.toString());
            cnt++;
          }
        }
      }
  ...
  public String getColumnTypeCommandString() {
      ...
      for (int ii = 0; ii < num; ii++) {
        ...
        if (equalFlag) {
          ...
          String columnTypeStr =
              SGDataColumnTitleUtility.appendColumnNo(LOWER_UPPER_ERROR_VALUE, ehIndex);
          columnTypeList.add(columnTypeStr);
        } else {
          String leStr = Integer.toString(leNo);
          varList.add(leStr);
          String leColumnTypeStr =
              SGDataColumnTitleUtility.appendColumnNo(LOWER_ERROR_VALUE, ehIndex);
          columnTypeList.add(leColumnTypeStr);

          String ueStr = Integer.toString(ueNo);
          varList.add(ueStr);
          String ueColumnTypeStr =
              SGDataColumnTitleUtility.appendColumnNo(UPPER_ERROR_VALUE, ehIndex);
    ...
    if (this.isTickLabelAvailable() && this.hasGenericTickLabels()) {
      for (int ii = 0; ii < this.mTickLabelIndices.length; ii++) {
        ...
        varList.add(str);
        String columnTypeStr = SGDataColumnTitleUtility.appendColumnNo(TICK_LABEL, thIndex);
        columnTypeList.add(columnTypeStr);
      }
    }
  ...
  public void addSingleDimensionEditedDataValue(SGDataValueHistory dataValue) {
    ...
    if (prev != null) {
      mdValue =
          new SGDataValueHistory.SDArray.MD1(
              dValue.getValue(), dValue.getColumnType(), 0, dValue.getIndex(), prev.getValue());
    } else {
      mdValue =
          new SGDataValueHistory.SDArray.MD1(
              dValue.getValue(), dValue.getColumnType(), 0, dValue.getIndex());
    }
