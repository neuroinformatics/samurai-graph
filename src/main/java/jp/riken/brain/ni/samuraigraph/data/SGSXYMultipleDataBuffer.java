...
public class SGSXYMultipleDataBuffer extends SGOneDimensionalDataBuffer {
  ...
  public SGSXYMultipleDataBuffer(
      double[] singleValues, double[][] multipleValues, final boolean yValuesMultipleFlag) {
    this();
    ...
    for (int ii = 0; ii < multipleValues.length; ii++) {
      ...
      if (singleValues.length != multipleValues[ii].length) {
        throw new IllegalArgumentException(
            "singleValues.length != multipleValues[" + ii + "].length");
      }
    }
  ...
  public SGSXYMultipleDataBuffer(double[][] xValues, double[][] yValues) {
    ...
    for (int ii = 0; ii < xValues.length; ii++) {
      ...
      if (length == -1) {
        ...
        if (length != xValues[ii].length) {
          throw new IllegalArgumentException(
              "length != xValues[" + ii + "].length: " + length + ", " + xValues[ii].length);
        }
      }
    }
    for (int ii = 0; ii < yValues.length; ii++) {
      ...
      if (length != yValues[ii].length) {
        throw new IllegalArgumentException(
            "length != yValues[" + ii + "].length: " + length + ", " + yValues[ii].length);
      }
    }
    this.mLength = length;
  ...
  public SGSXYMultipleDataBuffer(
      double[] singleValues,
      double[][] multipleValues,
      final boolean yValuesMultipleFlag,
      final Boolean dateFlag,
      SGDate[] dateArray,
      double[][] leValues,
      double[][] ueValues,
  ...
  public SGSXYMultipleDataBuffer(
    ...
    if (((leValues != null && ueValues != null) || (tickLabels != null)) && yHolderFlag == null) {
  ...
  public SGSXYDataBuffer[] getSXYDataBufferArray() {
    ...
    for (int ii = 0; ii < buffers.length; ii++) {
      Boolean sameErrorVariableFlag =
          (this.mSameErrorVariableFlags != null) ? this.mSameErrorVariableFlags[ii] : null;
   ...
   *
   * @param indices indices to pick up
  ...
  public SGSXYMultipleDataBuffer pickUp(final int[] indices) {
    ...
    double[][] lowerErrorValues =
        (this.mLowerErrorValues != null) ? new double[indices.length][] : null;
    double[][] upperErrorValues =
        (this.mUpperErrorValues != null) ? new double[indices.length][] : null;
    Boolean[] sameErrorVariableFlags =
    ...
    final SGSXYMultipleDataBuffer ret =
