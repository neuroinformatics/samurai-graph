...
public class SGElementGroupTickLabelInGraphSXY extends SGElementGroupTickLabelInGraph {

  /**
   * Builds the tick labels.
   *
   * @param dataSXY SXY type data
   * @param graph the graph
   */
  public SGElementGroupTickLabelInGraphSXY(
      SGISXYTypeSingleData dataSXY, SGFigureElementGraph graph) {
    super(dataSXY, graph);
   ...
   */
  public SGElementGroupTickLabelInGraphSXY(
      SGISXYTypeMultipleData dataSXY, SGFigureElementGraph graph) {
    super(dataSXY, graph);
   ...
   * Sets the location.
   *
   * @param pointArray array of the location
  ...
  protected boolean calcLocation(
    ...
    for (int ii = 0; ii < num; ii++) {
      final TickLabelStringElement el = (TickLabelStringElement) this.mDrawingElementArray[ii];
      final float location = this.mGraph.calcLocation(el.mValue, axis, alignHorizontal);
  ...
  public boolean contains(
      int x, int y, SGTuple2f[] pointsArray, final double[] xValues, final double[] yValues) {
  ...
  @Override
  public List<Integer> getIndicesAt(
      final int x,
      final int y,
