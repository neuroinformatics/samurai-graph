...
public abstract class SGAxisElement
  ...
  /** The format for date string. Initializes with an empty string. */
  ...
  List<AxisScaleChangeListener> mAxisScaleChangeListenerList =
      new ArrayList<AxisScaleChangeListener>();
  ...
  public boolean setMinorTickMarkLength(final float len, final String unit) {
    final Float lNew =
        SGUtility.calcPropertyValue(
            ...
            MINOR_TICK_MARK_LENGTH_MIN,
            MINOR_TICK_MARK_LENGTH_MAX,
  ...
  public boolean hasValidAxisValues(
      ...
      final Integer scaleType,
      final SGAxisValue baseValue,
      final SGAxisStepValue stepValue) {
  ...
  protected void drawAxisLine(final Graphics2D g2d) {
    for (int ii = 0; ii < this.mAxisLines.length; ii++) {
      ...
      final float width = this.getMagnification() * line.getLineWidth();
  ...
  protected void drawTitle(final Graphics2D g2d) {
    ...
    if (this.mTitle.equals(this.mAxisElement.mDraggableAxisScaleElement)) {
  ...
  protected void drawTickMarks(final Graphics2D g2d) {
    final boolean lineDragFlag =
        (this.mDraggingElement != null) && (this.mDraggingElement instanceof ElementLineTickMark);
    for (int ii = 0; ii < this.mTickMarksList.size(); ii++) {
      ...
      BasicStroke stroke = new BasicStroke(dWidth, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL);
      if (lineDragFlag) {
    ...
    if (this.isNumbersVisible()) {
      for (int ii = 0; ii < this.mNumberList.size(); ii++) {
        ...
        if (el.contains(x, y)) {
          ...
          this.mAxisElement.mPressedElementOrigin =
              new Point((int) (e.getX() - el.getX()), (int) (e.getY() - el.getY()));
  ...
  protected AxisValueRange dragScaleNumberHorizontal(MouseEvent e, ElementLineAxis line) {
    ...
    double draggedCoordinate =
        e.getX() - this.mAxisElement.mPressedElementOrigin.x + rect.getWidth() / 2.0;
  ...
  private boolean createScaleNumberInstances() {
    ...
    if (this.getDateMode()) {
      ...
      for (int ii = 0; ii < strArray.length; ii++) {
        String str = SGDateUtility.toStringByDateValue(vArray[ii], SGDateUtility.getUTCZoneId());
      ...
      if (scaleType == SGAxis.LINEAR_SCALE) {
        ...
        for (int ii = 0; ii < strArray.length; ii++) {
          ...
          strArray[ii] = SGUtilityText.getSuperscriptString("10", Integer.toString(order));
        }
      }
    }
    ...
    for (int ii = 0; ii < vArray.length; ii++) {
      ElementStringNumber el = this.createSingleNumberInstance(strArray[ii]);
      el.mValue = vArray[ii];
  ...
  protected ElementStringNumber createSingleNumberInstance(final String str) {
    ElementStringNumber el =
        new ElementStringNumber(
            str,
            this.mNumberFontName,
            this.mNumberFontStyle,
  ...
  protected ElementStringExponent createExponentStringElement(final String str) {
    ElementStringExponent el =
        new ElementStringExponent(
            this,
            str,
            this.mNumberFontName,
            this.mNumberFontStyle,
  ...
  protected boolean createTickMarksInLogScale(
    ...
    if (this.mAxisValueArray.length >= 1) {
      for (int jj = 2; jj < 10; jj++) {
        final double value = this.mAxisValueArray[this.mAxisValueArray.length - 1] * jj;
        this.createTicks(value, width, minorLen, cl);
      }
    }
  ...
  private void createTicks(final double value, final float width, final float len, final Color cl) {
  ...
  protected abstract void createTicksSub(
      final double value, final float width, final float len, final Color cl, final boolean inside);

  /**
   * Sets up the properties of created scale line.
   *
   * @param el a scale line
   * @param value axis value set to the scale line
   * @param cl the color of line
   */
  protected void setupCreatedScaleLineProperties(
      final ElementLineTickMark el, final double value, final Color cl) {
    el.mValue = value;
  ...
  protected SGPropertyResults setProperties(SGPropertyMap map, SGPropertyResults iResult) {
    ...
    while (itr.hasNext()) {
      ...
      if (COM_AXIS_VISIBLE.equalsIgnoreCase(key)) {
        ...
        Boolean b = SGUtilityText.getBoolean(value);
        if (b == null) {
          result.putResult(this.getLineVisibleCommandKey(), SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        if (this.setAxisLineVisible(b.booleanValue()) == false) {
          result.putResult(this.getLineVisibleCommandKey(), SGPropertyResults.INVALID_INPUT_VALUE);
        ...
        result.putResult(this.getLineWidthCommandKey(), SGPropertyResults.SUCCEEDED);
      } else if (this.getLineColorCommandKey().equalsIgnoreCase(key)) {
        Color cl = SGUtilityText.getColor(value);
        if (cl != null) {
          if (this.setAxisLineColor(cl) == false) {
            result.putResult(this.getLineColorCommandKey(), SGPropertyResults.INVALID_INPUT_VALUE);
            continue;
          }
        } else {
        ...
        if (num == null) {
          result.putResult(
              this.getSpaceLineAndNumberCommandKey(), SGPropertyResults.INVALID_INPUT_VALUE);
          continue;
        }
        if (this.setSpaceAxisLineAndNumbers(num.floatValue(), unit.toString()) == false) {
          result.putResult(
        ...
        result.putResult(this.getSpaceLineAndNumberCommandKey(), SGPropertyResults.SUCCEEDED);
  ...
  public boolean setMajorTickMarkLength(final float len, final String unit) {
    final Float lNew =
        SGUtility.calcPropertyValue(
            ...
            unit,
            TICK_MARK_LENGTH_UNIT,
            TICK_MARK_LENGTH_MIN,
            TICK_MARK_LENGTH_MAX,
  ...
  public String getCommandString() {
    ...
    String axisCommands =
        SGCommandUtility.createCommandString(
            COM_AXIS, this.getLocationName(), this.getPropertyMap());
  ...
  private void addAxisLineProperties(
      ...
      String spaceKey,
      String colorKey) {
    SGPropertyUtility.addProperty(map, lineVisibleKey, this.isAxisLineVisible());
    SGPropertyUtility.addProperty(
        map,
        lineWidthKey,
  ...
  private void addNumberProperties(
      ...
      String integerKey,
      String angleKey,
      String exponentVisibleKey,
      String exponentValueKey,
      String exponentXKey,
      String exponentYKey,
      String fontNameKey,
      String fontSizeKey,
    ...
    SGPropertyUtility.addProperty(
        ...
        exponentXKey,
        SGUtility.getExportValue(
            this.getExponentLocationX(EXPONENT_LOCATION_UNIT), EXPONENT_LOCATION_MINIMAL_ORDER),
        EXPONENT_LOCATION_UNIT);
    SGPropertyUtility.addProperty(
        map,
        exponentYKey,
        SGUtility.getExportValue(
            this.getExponentLocationY(EXPONENT_LOCATION_UNIT), EXPONENT_LOCATION_MINIMAL_ORDER),
        EXPONENT_LOCATION_UNIT);
  ...
  private void addTickMarkProperties(
      ...
      String bothsidesKey,
      String widthKey,
      String majorLengthKey,
      String minorLengthKey,
      String minorNumberKey,
      String colorKey) {
    ...
    SGPropertyUtility.addProperty(
        ...
        majorLengthKey,
        SGUtility.getExportValue(
            this.getMajorTickMarkLength(TICK_MARK_LENGTH_UNIT), TICK_MARK_LENGTH_MINIMAL_ORDER),
        TICK_MARK_LENGTH_UNIT);
    SGPropertyUtility.addProperty(
        map,
        minorLengthKey,
        SGUtility.getExportValue(
            this.getMinorTickMarkLength(TICK_MARK_LENGTH_UNIT), TICK_MARK_LENGTH_MINIMAL_ORDER),
        TICK_MARK_LENGTH_UNIT);
  ...
  protected void setScaleMinValue(
      SGPropertyMap map,
      String key,
      String value,
    ...
    if (dateMode) {
      ...
      if (date == null) {
        result.putResult(COM_AXIS_SCALE_MIN_VALUE, SGPropertyResults.INVALID_INPUT_VALUE);
        return;
      }
    ...
    result.putResult(COM_AXIS_SCALE_MIN_VALUE, SGPropertyResults.SUCCEEDED);
  }

  protected void setScaleMaxValue(
      SGPropertyMap map,
      String key,
      String value,
      final boolean dateMode,
    ...
    if (dateMode) {
      SGDate date = SGUtilityText.getDate(value);
      if (date == null) {
        result.putResult(COM_AXIS_SCALE_MAX_VALUE, SGPropertyResults.INVALID_INPUT_VALUE);
        return;
      }
      axisValue = new SGAxisDateValue(date);
    } else {
    ...
    result.putResult(COM_AXIS_SCALE_MAX_VALUE, SGPropertyResults.SUCCEEDED);
  }
  ...
  protected void setScaleStepValue(
      SGPropertyMap map,
      String key,
      String value,
      final boolean dateMode,
  ...
  public static class ElementStringTitle extends SGDrawingElementString2DExtended
    ...
    public void translate(final float dx, final float dy) {
      ...
      final float centerNew =
          this.mAxisElement.roundOffTitleShift(centerShift + factor * centerShiftDiff);
  ...
  public static class ElementStringNumber extends SGDrawingElementString2DExtended
      implements AxisScaleElement {
  ...
  public static class ElementStringExponent extends SGDrawingElementString2DExtended
    ...
    public ElementStringExponent(
        final SGAxisElement aElement,
        String str,
        String fontName,
        final int fontStyle,
        final float fontSize,
