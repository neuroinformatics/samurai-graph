...
public class SGMDArrayDataColumnSelectionPanel extends SGDataColumnSelectionPanel {
  ...
  public static final String[] COLUMN_NAME_ARRAY_SXY = {
    COLUMN_NAME_NUMBER,
    COLUMN_NAME_NAME,
    COLUMN_NAME_VALUE_TYPE,
    COLUMN_NAME_GENERIC_DIMENSION,
    COLUMN_NAME_TIME_DIMENSION,
    COLUMN_NAME_PICKED_UP_DIMENSION,
    COLUMN_NAME_COLUMN_TYPE
  };

  private static final int[] PREFERRED_COLUMN_WIDTH_ARRAY_SXY = {10, 80, 60, 50, 50, 50, 120};
  ...
  private static final int[] PREFERRED_COLUMN_WIDTH_ARRAY_MULTI_DIMENSION = {10, 80, 60, 40, 140};
  ...
  protected class MDArrayDataColumnTableModel extends DataColumnTableModel {
    ...
    protected void addData(int index, SGDataColumnInfo data) {
      ...
      Object[] array = {
        Integer.toString(index + 1), data.getTitle(), col.getValueType(), timeIndexStr, cType
      };
  ...
  protected class SXYMDArrayDataColumnTableModel extends DataColumnTableModel {
    ...
    protected void addData(int index, SGDataColumnInfo data) {
      ...
      if (pickUpIndex == null) {
        ...
        if (SGDataColumnInfoUtility.isValidPickUpValue(pickUpIndex)) {
  ...
  static String getTimeIndexString(SGMDArrayDataColumnInfo col) {
    ...
    if (timeIndex == null) {
      ...
      if (SGDataColumnInfoUtility.isValidTimeValue(timeIndex)) {
  ...
  public boolean checkSelectedItems() {
    ...
    for (int ii = 0; ii < colInfoList.size(); ii++) {
      ...
      if (indexSet.size() != indexList.size()) {
        this.setMessage(SGDataMiscUtility.MSG_UNIQUE_DIMENSIONS);
        return false;
      }
    }
    ...
    for (int ii = 0; ii < colInfoList.size(); ii++) {
      ...
      if (timeLen == -1) {
        ...
        if (timeLen != len) {
          this.setMessage(SGDataMiscUtility.MSG_DIMENSIONS_SAME_LENGTH_ANIMATION_FRAME);
          return false;
        }
      }
    }
    ...
    if (SGDataDataTypeUtility.isSXYTypeData(this.mDataType)) {
      ...
      if (pickUpList.size() > 0) {
        if (xList.size() > 1 || yList.size() > 1) {
          this.setMessage(SGDataMiscUtility.MSG_DIMENSION_AND_COLUMN_TYPE_PICK_UP);
          return false;
        }
      }
    }
  ...
  protected boolean setDefaultColumnType() {
    ...
    SGDefaultColumnTypeUtility.DefaultMDColumnTypeResult result =
        (DefaultMDColumnTypeResult)
            SGDefaultColumnTypeUtility.getDefaultColumnTypes(
                this.mDataType, this.getColumnInfoList(), this.mDataInfoMap);
  ...
  public boolean setData(
    ...
    if (SGDataDataTypeUtility.isSXYTypeData(this.mDataType)) {
      ...
      TableColumn dimensionCol = this.mTable.getColumnModel().getColumn(dimensionColIndex);
      dimensionCol.setCellEditor(
          new GenericDimensionCellEditor(this.mGenericDimensionEditorComboBox));
      dimensionCol.setCellRenderer(
          new DataColumnCellRenderer(this.mGenericDimensionRendererComboBox));

      final int pickUpColIndex = this.getColumnIndex(COLUMN_NAME_PICKED_UP_DIMENSION);
      TableColumn pickUpCol = this.mTable.getColumnModel().getColumn(pickUpColIndex);
  ...
  protected class GenericDimensionCellEditor extends DefaultCellEditor {
    ...
    public Component getTableCellEditorComponent(
      ...
      DimensionEditorComboBox cb =
          (DimensionEditorComboBox)
  ...
  protected void syncDataColumnInfoWithTable(final int rowIndex) {
    ...
    if (SGDataDataTypeUtility.isSXYTypeData(this.mDataType)) {
      // generic
      Object objDimension =
          this.getValueAt(rowIndex, this.getColumnIndex(COLUMN_NAME_GENERIC_DIMENSION));
      Integer dimension = SGUtilityText.getInteger(objDimension.toString());
      info.setGenericDimensionIndex(dimension);

      // pick up
      Object objPickUpDimension =
          this.getValueAt(rowIndex, this.getColumnIndex(COLUMN_NAME_PICKED_UP_DIMENSION));
  ...
  public void setDimensionIndex(
      final int rowIndex, final String key, final Integer dimensionIndex) {
    if (dimensionIndex == null) {
      return;
    }
