...
public class SGJarClassLoader extends ClassLoader {
  ...
  private void definePackage(String className) throws IOException {
    ...
    if (idx != -1) {
      ...
      if (!found) {
        definePackage(
            name, null, null, null, null, null, null, null); // define package by using its name.
        return;
      }
    }
  }
   ...
   * @throws ClassNotFoundException
   */
  private Class<?> loadFromParent(final String className) throws ClassNotFoundException {
    Class<?> cls;
