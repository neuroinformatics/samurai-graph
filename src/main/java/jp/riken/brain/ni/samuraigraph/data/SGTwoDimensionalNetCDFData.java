...
public abstract class SGTwoDimensionalNetCDFData extends SGNetCDFData
   ...
   * @param xIndexInfo information of index for x-values
   * @param yIndexInfo information of index for y-values
   * @param indexStride stride for the index
   * @param xStride stride for the x values
   * @param yStride stride for the y values
    ...
    if (this.isIndexAvailable()) {
      ...
      if (xIndexInfo != null && xIndexVar != null) {
        ...
        if (!xVar.isCoordinateVariable()) {
          throw new IllegalArgumentException(
              "X variable must be a coordinate variable: " + xVar.getName());
        }
  ...
  protected double[] arrangeValueArray(
    ...
    if (this.isIndexAvailable()) {
      return this.arrangeValueArray1Dim(
          blockList, this.mIndexVariable, this.getIndexStrideInstance());
    } else {
      if (var.equals(this.mXVariable)) {
        return this.arrangeValueArray1Dim(blockList, var, this.getXIndexStrideInstance());
      } else if (var.equals(this.mYVariable)) {
        return this.arrangeValueArray1Dim(blockList, var, this.getYIndexStrideInstance());
      } else {
        return this.arrangeValueArray2Dim(
            blockList,
            this.mXVariable,
            this.mYVariable,
            this.getXStrideInstance(),
            this.getYStrideInstance());
      }
    }
  }
  ...
  public boolean setData(SGData data) {
    if (!(data instanceof SGTwoDimensionalNetCDFData)) {
      throw new IllegalArgumentException(
          "Given data is not the instance of SGTwoDimensionalNetCDFData class.");
    }
  ...
  protected Map<String, SGIntegerSeriesSet> getDimensionStrideMap() {
    ...
    if (this.isIndexAvailable()) {
      ...
      String xName =
          this.isXIndexAvailable() ? this.mXIndexVariable.getName() : this.mXVariable.getName();
      String yName =
          this.isYIndexAvailable() ? this.mYIndexVariable.getName() : this.mYVariable.getName();
  ...
  protected List<SGXYSimpleDoubleValueIndexBlock> getValueBlockList(
      SGNetCDFVariable var, final boolean all, final boolean removeInvalidValues) {
    ...
    List<SGXYSimpleDoubleValueIndexBlock> blockList =
        this.get2DValueBlockList(var, xVar, yVar, all, removeInvalidValues);
    return blockList;
  }

  /** A class of two-dimensional NetCDF data properties. */
  public abstract static class TwoDimensionalNetCDFDataProperties extends NetCDFDataProperties {
  ...
  protected double[] getXYValueArray(
      final boolean all,
      SGNetCDFVariable xyVar,
      SGNetCDFVariable xyCounterVar,
    ...
    if (this.isIndexAvailable()) {
      ret =
          this.getValueArray(
              xyVar, new SGNetCDFVariable[] {this.mIndexVariable}, all, removeInvalidValues);
    } else {
      if (xyVar.isCoordinateVariable()) {
        ret = this.getCoordinateValueArray(xyVar, indices, all, removeInvalidValues);
      } else {
        ret =
            this.getValueArray(
                xyVar, new SGNetCDFVariable[] {xyCounterVar}, all, removeInvalidValues);
      }
    }
    return ret;
  }
  ...
  protected SGTwoDimensionalArrayIndex getDataViewerCellX(
      SGTwoDimensionalArrayIndex cell, final boolean bStride) {
  ...
  protected SGTwoDimensionalArrayIndex getDataViewerCellY(
      SGTwoDimensionalArrayIndex cell, final boolean bStride) {
    SGTwoDimensionalArrayIndex ret;
    if (bStride) {
