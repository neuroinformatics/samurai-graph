...
public class SGDrawingElementString2DExtended extends SGDrawingElementString2D {
  ...
  private boolean createStringElements() {
    ...
    if (!this.getSubscriptAndSuperscriptInfo(line, baseList, superList, subList)) {
  ...
  protected boolean getSubscriptAndSuperscriptInfo(
    ...
    return SGUtilityText.getSubscriptAndSuperscriptInfo(line, baseList, superList, subList);
  }
    ...
    for (int ii = 0; ii < this.mBaseElementList.size(); ii++) {
      ...
      float super_advance = 0.0f;
      {
        ...
        if (super_el != null) {
          ...
          final float super_visual_height = (float) super_el.getStringRect().getHeight();
          // calculate location
      ...
      // increase advance
      advance += base_advance + ((super_advance > sub_advance) ? super_advance : sub_advance);
    }
  ...
  private void calcMetrics() {
    ...
    // calculate ascent
    {
      final float center_top = base_visual_ascent + base_strike_through_offset;
      final float tmp1 = super_height + margin;
      if (tmp1 > center_top) visual_ascent = tmp1 - base_strike_through_offset;
      else visual_ascent = center_top - base_strike_through_offset;
    }

    // calculate descent
    {
      final float center_bottom = base_visual_descent - base_strike_through_offset;
      final float tmp2 = sub_height + margin;
      if (tmp2 > center_bottom) visual_descent = tmp2 + base_strike_through_offset;
      else visual_descent = center_bottom + base_strike_through_offset;
    }
