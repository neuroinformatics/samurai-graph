...
class SGAxisScale extends SGDrawingElementScale2D
  ...
  private boolean drag(final MouseEvent e) {
    ...
    mAxisElement.setPressedPointLocation(pos.getX() + diffX, pos.getY() + diffY);
  ...
  private void onLabelClicked(MouseEvent e, ScaleString el) {
    ...
    if (SwingUtilities.isLeftMouseButton(e) && cnt == 1) {
      if (this.isSelected() && !ctrl && !shift) {
        String unitStr =
            this.getHorizontalStringElement().equals(el)
        ...
        if (this.isValidText(unitStr)) {
          ...
          elUnit.setLocation(
              (float) (sRect.getX() + sRect.getWidth() - unitRect.getWidth()),
              (float) sRect.getY());
    ...
    if (AXIS_LENGTH_MODE.VALUE_FIXED.equals(this.mAxisLengthMode)) {
      ...
      if (this.mXAxis.equals(axis)) {
        if (this.mHorizontalText == null) {
          ...
          this.mXLengthValue = SGUtilityNumber.getNumberInRangeOrder(diffValue, this.mXAxis);
        }
      } else if (this.mYAxis.equals(axis)) {
        if (this.mVerticalText == null) {
          final double diffValue = this.getDiffValue(false);
          this.setNumberLabel(diffValue, false);
          this.mYLengthValue = SGUtilityNumber.getNumberInRangeOrder(diffValue, this.mYAxis);
        }
      }
      this.updateNumberLabelsWithLocation();
      ...
      if (this.mXAxis.equals(axis)) {
        ...
        this.mXLengthValue = SGUtilityNumber.getNumberInRangeOrder(sign * stepValue, this.mXAxis);
      } else if (this.mYAxis.equals(axis)) {
        final int sign = this.mYLengthValue >= 0.0 ? 1 : -1;
        this.mYLengthValue = SGUtilityNumber.getNumberInRangeOrder(sign * stepValue, this.mYAxis);
      }
      this.updateSizeWithAxisValues();
      this.updateNumberLabelsWithLocation();
    }
  ...
  private void setNumberLabel(final double value, final boolean horizontal) {
    ...
    final double valueNew =
        (value == 0.0 || Double.isNaN(value))
            ? value
            : SGUtilityNumber.getNumberInRangeOrder(value, axis);
  ...
  public boolean hasValidXValue(final int location, final Number value) {
    final SGAxis axis = (location == -1) ? this.mXAxis : this.mAxisElement.getAxisInPlane(location);
  ...
  public boolean hasValidYValue(final int location, final Number value) {
    final SGAxis axis = (location == -1) ? this.mYAxis : this.mAxisElement.getAxisInPlane(location);
  ...
  boolean updateSizeWithAxisValues() {
    ...
    final float y = this.mAxisElement.calcLocation(this.mYValue, this.mYAxis, false);
    final float endX =
        this.mAxisElement.calcLocation(this.mXValue + this.mXLengthValue, this.mXAxis, true);
    final float endY =
        this.mAxisElement.calcLocation(this.mYValue + this.mYLengthValue, this.mYAxis, false);
